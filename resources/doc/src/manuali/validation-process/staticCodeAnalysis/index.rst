...
Dopo aver lavorato su un branch dedicato alla realizzazione di una nuova funzionalità o di un bug fix, prima di riportare il lavoro sul `branch 3.4.x dei sorgenti del progetto <https://github.com/link-it/govway/tree/3.4.x/>`_ i sorgenti soggetti a modifica vengono verificati anche tramite il tool *SonarQube* (:ref:`releaseProcessGovWay_staticCodeAnalysis_sonarqube_eclipse`).

Ad ogni commit sul `branch 3.4.x dei sorgenti del progetto <https://github.com/link-it/govway/tree/3.4.x/>`_ viene effettuata automaticamente una verifica dei sorgenti nell'ambiente di `Continuous Integration Jenkins di GovWay <https://jenkins.link.it/govway4/job/GovWay/>`_ utilizzando entrambi i tools. Maggiori dettagli vengono forniti nelle sezioni :ref:`releaseProcessGovWay_staticCodeAnalysis_spotbugs_ci` e :ref:`releaseProcessGovWay_staticCodeAnalysis_sonarqube_ci`.
