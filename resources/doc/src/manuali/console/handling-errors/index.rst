...
- *Errori Client*: sono identificabili da un codice http 4xx su API REST o da un fault code 'Client' su API SOAP. Indicano che GovWay ha rilevato problemi nella richiesta effettuata dal client (es. errore autenticazione, autorizzazione, validazione contenuti...). Il problema deve essere risolto dal Client prima di effettuare una nuova richiesta.
...
.. note::
      I SOAP Fault generati da GovWay vengono veicolati su codice http 5xx
...
Una gestione degli errori permette al client di comprendere se l'errore è avvenuto a causa di una richiesta malformata o meno; nel caso di errore non riconducibile al formato della richiesta il client potrebbe pianificare un nuovo invio. Maggiori dettagli sulla logica di re-invio delle richieste vengono descritti nella sezione :ref:`gestioneErrori`.
