	            ...
	            <xsd:element name="pseudonymizationResponse">
	                <xsd:complexType>
	                    <xsd:sequence>
	                        ...
	                        <xsd:element name="signalId" type="xsd:long" minOccurs="0"/>
	                    </xsd:sequence>
	                </xsd:complexType>
	            </xsd:element>
...
L'elemento opzionale ``signalId`` esposto nella risposta indica il signalId del segnale di tipo ``SEEDUPDATE`` che ha introdotto il seme corrente; consente al consumatore di sincronizzarsi correttamente sapendo da quale segnale in poi il seme è applicabile. Per il primo seme storico (mai sostituito da un ``SEEDUPDATE``) il valore esposto è ``0``. L'esposizione dell'elemento è controllata dalla proprietà ``org.openspcoop2.protocol.modipa.signalHub.pseudonymization.exposeSignalId`` descritta nella sezione :ref:`Esposizione del signalId sul servizio di pseudoanonimizzazione <modipa_signalhub_exposeSignalId>`.
