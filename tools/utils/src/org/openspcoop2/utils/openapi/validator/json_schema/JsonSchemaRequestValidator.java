...
public class JsonSchemaRequestValidator extends AbstractApiValidator implements IApiValidator {
	...
	public void init(Logger log, Api api, ApiValidatorConfig rawConfig) throws ProcessingException {
		...
		String cacheKey = this.config.cacheKey(OpenAPILibrary.json_schema.name());
		...
		// Verifichiamo anche che la struttura sia effettivamente utilizzabile da json_schema
		// (validatorMap valorizzato): porte sullo stesso accordo con engine diversi popolano cache
		// strutturate diversamente e l'unico modo per distinguerle - se la chiave coincide per qualche
		// motivo - è controllare la presenza dei campi attesi da questo engine.
		...
		if (apiValidatorStructure != null
				&& (apiValidatorStructure.getValidatorMap() == null
						|| apiValidatorStructure.getValidatorMap().isEmpty())) {
			// Cache non utilizzabile per json_schema: ricostruiamo.
