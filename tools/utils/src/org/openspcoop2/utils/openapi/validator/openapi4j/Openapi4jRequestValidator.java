...
public class Openapi4jRequestValidator extends AbstractApiValidator implements IApiValidator {
	...
	private Map<String, org.openapi4j.parser.model.v3.MediaType> getContentMediaTypes(Operation operation, Integer statusRisposta) {
		if (statusRisposta == null) {
			org.openapi4j.parser.model.v3.RequestBody requestBodyOpenApi4j = operation.getRequestBody();
			return requestBodyOpenApi4j != null ? requestBodyOpenApi4j.getContentMediaTypes() : null;
		}
		org.openapi4j.parser.model.v3.Response responseOpenApi4j = operation.getResponse(statusRisposta.toString());
		if (responseOpenApi4j == null) {
			responseOpenApi4j = operation.getResponse(OAI3SchemaKeywords.DEFAULT);
		}
	...
	private boolean isBodyValidabile(Operation operation, String contentType, Integer statusRisposta) {
		try {
			...
			Map<String, org.openapi4j.parser.model.v3.MediaType> mediaTypes = getContentMediaTypes(operation, statusRisposta);
			...
			return OAI3SchemaKeywords.TYPE_OBJECT.equals(tipo) || OAI3SchemaKeywords.TYPE_ARRAY.equals(tipo);
		...
		try {
			...
			if (httpEntity instanceof HttpBaseRequestEntity<?> httpRequest) {
				...
				if (this.config.isValidateRequestBody()
						&& isBodyValidabile(operation, httpRequest.getContentType(), null)) {
				...
				if (this.config.isValidateResponseBody()
						&& isBodyValidabile(operation, response.getContentType(), response.getStatus())) {
