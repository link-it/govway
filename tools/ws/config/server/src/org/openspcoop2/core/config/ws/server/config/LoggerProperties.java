...
public class LoggerProperties {
	...
	public static void initialize(Logger logConsole,String rootDirectory,Properties objectProperties) throws IOException{
		...
		if(rootDirectory!=null)
			loggerFile = new java.io.File(rootDirectory+"wsconfig.log4j.properties");
		else
			loggerFile = new java.io.File("wsconfig.log4j.properties");
		if(loggerFile .exists() == false ){
			loggerProperties.load(LoggerProperties.class.getResourceAsStream("/wsconfig.log4j.properties"));
		}else{
	...
	public static Logger getLoggerWS(){
		...
		return Logger.getLogger("config.ws");
	}
	
	public static Logger getLoggerDAO(){
		return Logger.getLogger("config.dao");
	}
}
