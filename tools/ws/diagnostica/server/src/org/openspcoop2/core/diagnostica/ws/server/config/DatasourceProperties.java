...
public class DatasourceProperties {	
	...
	/** Reader delle proprieta' impostate nel file 'wsdiagnostica.datasource.properties' */
	...
	public DatasourceProperties(String confDir,Logger log) throws Exception {
		...
		try{  
			properties = DatasourceProperties.class.getResourceAsStream("/wsdiagnostica.datasource.properties");
			if(properties==null){
				throw new Exception("File '/wsdiagnostica.datasource.properties' not found");
			}
			propertiesReader.load(properties);
		}catch(Exception e) {
			this.log.error("Riscontrato errore durante la lettura del file 'wsdiagnostica.datasource.properties': \n\n"+e.getMessage());
		    throw new Exception("WSDiagnosticaProperties initialize error: "+e.getMessage());
		}finally{
		    try{
				if(properties!=null)
				    properties.close();
