...
public class LoggerProperties {
	...
	public static void initialize(Logger logConsole,String rootDirectory,Properties objectProperties) throws IOException, UtilsException{
		...
		if(rootDirectory!=null)
			loggerFile = new java.io.File(rootDirectory+"wsregistry.log4j2.properties");
		else
			loggerFile = new java.io.File("wsregistry.log4j2.properties");
		if(loggerFile .exists() == false ){
			loggerProperties.load(LoggerProperties.class.getResourceAsStream("/wsregistry.log4j2.properties"));
		}else{
	...
	public static Logger getLoggerWS(){
		...
		return LoggerWrapperFactory.getLogger("registry.ws");
	}
	
	public static Logger getLoggerDAO(){
		return LoggerWrapperFactory.getLogger("registry.dao");
	}
}
