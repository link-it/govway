...
public class DatasourceProperties {	
	...
	/** Reader delle proprieta' impostate nel file 'wstracciamento.datasource.properties' */
	...
	public DatasourceProperties(String confDir,Logger log) throws Exception {
		...
		try{  
			properties = DatasourceProperties.class.getResourceAsStream("/wstracciamento.datasource.properties");
			if(properties==null){
				throw new Exception("File '/wstracciamento.datasource.properties' not found");
			}
			propertiesReader.load(properties);
		}catch(Exception e) {
			this.log.error("Riscontrato errore durante la lettura del file 'wstracciamento.datasource.properties': \n\n"+e.getMessage());
		    throw new Exception("WSTracciamentoProperties initialize error: "+e.getMessage());
		}finally{
		    try{
				if(properties!=null)
				    properties.close();
