...
public class ApplicativiApiHelper {
	...
	public static final Map<String,String> tipoProtocolloFromModalita = new HashMap<String,String>();
	static {	//TODO: Usare le costanti
		tipoProtocolloFromModalita.put("APIGateway", "trasparente");
		tipoProtocolloFromModalita.put("SPCoop", "spcoop");
		tipoProtocolloFromModalita.put("FatturaPA", "sdi");
		tipoProtocolloFromModalita.put("eDelivery", "as4");
		
	}
	
	
	public static final Map<String,String> modalitaFromTipoProtocollo = new HashMap<String,String>();
	static {
		tipoProtocolloFromModalita.forEach( (mod,prot) -> modalitaFromTipoProtocollo.put(prot, mod));
	}
	...
	public static ServizioApplicativo applicativoToServizioApplicativo(Applicativo applicativo, String tipo_protocollo, String soggetto) throws UtilsException, Exception {
		...
		Credenziali credenziali = credenzialiFromAuth(applicativo.getCredenziali(), tipoAuthSAFromModalita.get(applicativo.getModalitaAccesso().toString()));

		invocazionePorta.addCredenziali(credenziali);
		
		final String fault = "soap"; //TODO: Potrebbe essere anche xml, ma in che casi?
	...
	public static final Applicativo servizioApplicativoToApplicativo(ServizioApplicativo sa) {
		...
		if (invPorta != null) {
			...
			if (ruoli != null) {
				ret.setRuoli(ruoli.getRuoloList().stream().map( ruolo -> ruolo.toString() ).collect(Collectors.toList()));
			}
		}
	
		
		Credenziali cred = invPorta.getCredenziali(0);
		ret.setModalitaAccesso(modalitaAccessoFromCredenzialeTipo.get(cred.getTipo()));
	...
	public static final Soggetto getSoggetto(String nome, String modalita, SoggettiCore soggettiCore) throws DriverRegistroServiziException, DriverRegistroServiziNotFound, ProtocolException {
		return soggettiCore.getSoggettoRegistro(getIDSoggetto(nome,tipoProtocolloFromModalita.get(modalita)));
	...
	public static Object translateCredenzialiApplicativo(Applicativo applicativo) {
		Object creds = null;
		String  tipoauthSA = tipoAuthSAFromModalita.get(applicativo.getModalitaAccesso().toString());
	...
	public static void checkApplicativoDuplicati(ServizioApplicativo sa, ServiziApplicativiCore saCore) throws DriverConfigurazioneException {
		...
		// Controllo che il servizioApplicativo non sia gia' stato registrato
		if (saCore.existsServizioApplicativo(idSA)) {
	...
	public static String getProtocolloOrDefault(ProfiloEnum profilo) throws UtilsException {
		...
		if (modalita == null || modalita.length() == 0) {
			...
			tipo_protocollo = tipoProtocolloFromModalita.get(modalita);
		}
