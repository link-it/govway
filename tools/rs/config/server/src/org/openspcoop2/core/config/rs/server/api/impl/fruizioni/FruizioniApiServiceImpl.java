...
public class FruizioniApiServiceImpl extends BaseImpl implements FruizioniApi {
	...
	private jakarta.ws.rs.WebApplicationException newNotFoundSoggettoFruitore(ErogazioniEnv env){
		...
		try {
			...
			if(profilo != null) {
				...
				if(env.isProfiloModi()) {
					...
					ModiErogazioniApiHelper.validateDPoPConfigurationCreate(body.getModiDpop(), body, env);
				}
