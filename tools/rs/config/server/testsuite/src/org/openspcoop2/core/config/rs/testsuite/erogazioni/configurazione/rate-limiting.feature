...
@Validazione
Scenario: Configurazione Erogazioni Rate Limiting

    * call create ({ resourcePath: 'api', body: api_petstore })
    * call create ({ resourcePath: 'erogazioni', body: erogazione_petstore })

    * def conf = call read('classpath:servizi-configurazione/rate-limiting.feature') ({servizio_path: erogazione_petstore_path, policy: policy, policy_update: policy_update })
