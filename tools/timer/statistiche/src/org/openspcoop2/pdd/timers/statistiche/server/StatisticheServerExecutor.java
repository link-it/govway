...
public class StatisticheServerExecutor {
	...
	private void initSemaphores() {
		...
		try {
			TipiDatabase databaseType = TipiDatabase.toEnumConstant(DAOFactoryProperties.getInstance(this.logCore).getTipoDatabase(org.openspcoop2.core.statistiche.utils.ProjectInfo.getInstance()));
	...
	private void eseguiJob(TipoIntervalloStatistico tipoStatistica) {
		...
		try {
			...
			if(!isJobEnabled(tipoStatistica)) {
			...
			if(logStatistica!=null) {
				logStatistica.error("Errore durante esecuzione job statistiche " + tipoStr + ": " + e.getMessage(), e);
			}
		} finally {
			// Rilascia lock
	...
	private StatisticsConfig createStatisticsConfig(TipoIntervalloStatistico tipoStatistica,
			Logger logStatistica, Logger logStatisticaSql) throws EngineException, UtilsException {
