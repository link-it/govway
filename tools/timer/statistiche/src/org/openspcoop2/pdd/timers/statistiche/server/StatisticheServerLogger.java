...
public class StatisticheServerLogger {
	...
	public static void initialize(Logger logConsole,String rootDirectory,Properties objectProperties) throws IOException, UtilsException{
		...
		if(rootDirectory!=null)
			loggerFile = new java.io.File(rootDirectory+StatisticheCostanti.LOG4J_PROPERTIES);
		else
			loggerFile = new java.io.File(StatisticheCostanti.LOG4J_PROPERTIES);
		if(!loggerFile .exists() ){
			loggerProperties.load(StatisticheServerLogger.class.getResourceAsStream("/"+StatisticheCostanti.LOG4J_PROPERTIES));
		}else{
		...
		readLocalImplementation(logConsole, rootDirectory, loggerProperties);
		
		// File Object Implementation
		readObjectImplementation(objectProperties, loggerProperties);
		...
		LoggerWrapperFactory.patchLoggers(loggerProperties,
				...
				Map.of(org.openspcoop2.utils.Costanti.VAR_LOGGER_APPNAME, "govwayStats"));
		
		LoggerWrapperFactory.setLogConfiguration(loggerProperties);
	}
	private static void readLocalImplementation(Logger logConsole, String rootDirectory, java.util.Properties loggerProperties) {
		CollectionProperties loggerPropertiesRidefinito =  
				PropertiesUtilities.searchLocalImplementation(Costanti.OPENSPCOOP2_LOCAL_HOME,logConsole, 
						StatisticheCostanti.LOG4J_PROPERTIES_LOCAL_VARIABLE,
						StatisticheCostanti.LOG4J_PROPERTIES_LOCAL,
						rootDirectory);
	...
	private static void readObjectImplementation(Properties objectProperties, java.util.Properties loggerProperties) {
