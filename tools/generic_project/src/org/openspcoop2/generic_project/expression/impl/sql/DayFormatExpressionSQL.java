 ...
 * @version $Rev$, $Date$
 */
public class DayFormatExpressionSQL extends DayFormatExpressionImpl implements ISQLExpression {

	private ISQLFieldConverter sqlFieldConverter;
	
	public DayFormatExpressionSQL(ISQLFieldConverter sqlFieldConverter,IObjectFormatter objectFormatter, IField field,
			String value, DayFormatEnum dayFormatEnum) {
		super(objectFormatter, field, value, dayFormatEnum);
	...
	private String toSqlEngine(SQLMode mode,ISQLQueryObject sqlQueryObject,List<?> oggettiPreparedStatement,Map<String, ?> oggettiJPA)throws ExpressionException{
		...
		try {
			String prefix = sqlObjectCore.getExtractDayFormatFromTimestampFieldPrefix(this.dayFormatEnum);
			bf.append(prefix);
		}catch(Exception e) {
			throw new ExpressionException(e.getMessage(),e);
		}
		...
		try {
			String suffix = sqlObjectCore.getExtractDayFormatFromTimestampFieldSuffix(this.dayFormatEnum);
			bf.append(" ");
			bf.append(suffix);
		}catch(Exception e) {
			throw new ExpressionException(e.getMessage(),e);
