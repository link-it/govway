...
function lineWrap(){
   var wrap = function () {
  		...
  		for (var j = 0; j < elems.length; ++j) {
  			var sh = elems[j];
  			if(sh) {
	   			var gLines = sh.getElementsByClassName('gutter').length > 0 ? sh.getElementsByClassName('gutter')[0].getElementsByClassName('line') : [];
	   			var cLines = sh.getElementsByClassName('code').length > 0 ? sh.getElementsByClassName('code')[0].getElementsByClassName('line') : [];
	   			var stand = 15;
...
function checkWidth(xmlPnl,bodyWrapperPanel)
{
	...
	if((offsetLeftXmlPnl + widthXmlPnl) > (offsetLeftBody +bodyWidth)){
		var diff = (offsetLeftXmlPnl + widthXmlPnl) - (offsetLeftBody +bodyWidth); // + 80;
		xmlPnl.width(widthXmlPnl - diff);
    }
...
function stripCData(original)
{
	...
	if ((copyLength - rightLength) > 0 && copy.indexOf(right) == copyLength - rightLength)
	{
