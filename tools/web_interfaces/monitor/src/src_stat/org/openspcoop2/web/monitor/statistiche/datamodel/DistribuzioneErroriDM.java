 ...
 *
 */
public class DistribuzioneErroriDM extends BaseDataModel<String, ResDistribuzione, IStatisticheGiornaliere> {
	...
	public int getRowCount() {
		try {
			...
			int count = this.getDataProvider().countAllDistribuzioneErrori();
			...
			DistribuzioneErroriDM.log.error(e.getMessage(), e);
			return 0;
		}
	}
	...
	public void walk(FacesContext context, DataVisitor visitor, Range range,
		...
		try{	
			if(this.detached){
				...
				try {
					list =  this.getDataProvider().findAllDistribuzioneErrori(start, limit);
				} catch (ServiceException e) {
					DistribuzioneErroriDM.log.error(e.getMessage(), e);
				}
