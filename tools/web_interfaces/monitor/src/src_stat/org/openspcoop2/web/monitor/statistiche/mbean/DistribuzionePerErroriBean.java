 ...
 *
 */
public class DistribuzionePerErroriBean<T extends ResBase> extends
BaseStatsMBean<T, Integer, IService<ResBase, Integer>> {
	...
	public String getJson(){
		...
		case BAR_CHART: {
			...
			case DIMENSIONI_3:
				StatisticType statisticType = StatsUtils.checkStatisticType((StatsSearchForm) this.search);
				grafico = JsonStatsUtils.getJsonHeatmapChartDistribuzione(list,(StatsSearchForm) this.search, this.getCaption(), this.getSubCaption(), this.getDirezioneLabel(), this.getSlice(), statisticType);
				break;
			case DIMENSIONI_2:
			default:
	...
	public String getCaption() {
		...
		sb.append(MessageManager.getInstance().getMessage(StatisticheCostanti.STATS_ANALISI_STATISTICA_TIPO_DISTRIBUZIONE_ERRORI_LABEL_SUFFIX_KEY)).append(CostantiGrafici.WHITE_SPACE);
		return sb.toString();
	}
	...
	public void newSearch(ActionEvent ae) {
		...
		ValueExpression valueExp = elFactory.createValueExpression(elContext,
				"#{distribuzionePerErroriBean}",
				DistribuzionePerErroriBean.class);
		DistribuzionePerErroriBean<ResDistribuzione> ab = new DistribuzionePerErroriBean<ResDistribuzione>();
	...
	private String _esportaCsv(HttpServletResponse responseParam, boolean useFaceContext) throws Exception {
		...
		try {	
			...
			String headerLabel = CostantiGrafici.ESITO_LABEL;
	...
	public String getExportFilename() {
		return CostantiGrafici.DISTRIBUZIONE_ERRORI_FILE_NAME;
	}
