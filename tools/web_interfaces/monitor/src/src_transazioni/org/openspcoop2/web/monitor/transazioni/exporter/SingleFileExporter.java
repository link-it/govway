		...
		if(dump!=null){
			...
			if(dump.getBody()!=null || (dump.getContentLength()!=null && dump.getContentLength()>0)){
				try{
					...
					if(dump.getBody()!=null) {
						byte[] contenutoBody = dump.getBody();
			...
			if(dumpMessaggio.getHeadersValues() != null && dumpMessaggio.getHeadersValues().size()>0){
				headers = new ArrayList<DumpHeaderTrasporto>();
				
				Map<String, String> toMapSingleValue = TransportUtils.convertToMapSingleValue(dumpMessaggio.getHeadersValues());
