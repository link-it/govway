...
//			}
	...
	public static void exportContenuti(Logger log, Transazione t, ZipOutputStream zip,String dirPath,ITransazioniService service,TipoMessaggio tipo,
		...
		try {
			dump = service.getDumpMessaggio(t.getIdTransazione(), null, tipo);
		} catch (Exception e) {
		...
		if(dump!=null){
			...
			//header trasporto
			List<DumpHeaderTrasporto> headers = service.getHeaderTrasporto(dump.getIdTransazione(), dump.getServizioApplicativoErogatore(), dump.getTipoMessaggio(), dump.getId());
			if(headers.size()>0){
				try{
					String name = "headers.";
					if(headersAsProperties) {
			...
			//contenuti
			List<DumpContenuto> contenuti = service.getContenutiSpecifici(dump.getIdTransazione(), dump.getServizioApplicativoErogatore(), dump.getTipoMessaggio(), dump.getId());
			if(contenuti.size()>0){
				try{
			...
			//allegati
			List<DumpAllegato> allegati = service.getAllegatiMessaggio(dump.getIdTransazione(), dump.getServizioApplicativoErogatore(), dump.getTipoMessaggio(), dump.getId());
			if(allegati.size()>0){
				try{
