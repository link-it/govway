...
public class MonitorCoreAuditManager {
	...
	private static final String AUDIT_LOG4J2_LOCAL_PROPERTIES_MONITOR = "monitor_local.audit.log4j2.properties";
	private static final String AUDIT_LOG4J2_ENV_MONITOR = "OPENSPCOOP2_MONITOR_AUDIT_LOG_PROPERTIES";
		...
		for(int i=0; i<configurazioneAuditing.sizeAppender(); i++){
			...
			if(AuditLog4JAppender.class.getName().equals(appender.getClassName())){
				for(int j=0; j<appender.sizeProperties(); j++){
					...
					else if(Costanti.AUDIT_APPENDER_LOG4J_PROPERTY_NOME_FILE_LOADER_INSTANCE.equals(p.getName())){
						p.setValue(AUDIT_LOG4J2_LOCAL_PROPERTIES_MONITOR);
					}
					else if(Costanti.AUDIT_APPENDER_LOG4J_PROPERTY_NOME_PROPRIETA_LOADER_INSTANCE.equals(p.getName())){
						p.setValue(AUDIT_LOG4J2_ENV_MONITOR);
					}
				}
			}
		}
