...
do_login() {
	...
	curl -s -X POST \
		...
		-H "Cookie: ${cookie}" \
		-d "javax.faces.ViewState=j_id1&j_id50=j_id50&AJAXREQUEST=_viewRoot&username=${username}&password=${password}&loginBtn=loginBtn&_csrf=${csrf}" \
...
curl -s -X POST \
	...
	-H 'Referer: '"${BASEURL}"'/public/login.jsf' \
	-H "Cookie: ${COOKIE1}" \
	-d "javax.faces.ViewState=j_id1&j_id50=j_id50&AJAXREQUEST=_viewRoot&username=${USERNAME}&password=${PASSWORD}&loginBtn=loginBtn&_csrf=${CSRF1}" \
...
curl -s -X POST \
	...
	-H 'Referer: '"${BASEURL}"'/public/login.jsf' \
	-H "Cookie: ${COOKIE_NEW}" \
	-d "javax.faces.ViewState=j_id1&j_id50=j_id50&AJAXREQUEST=_viewRoot&username=${USERNAME}&password=${PASSWORD}&loginBtn=loginBtn&_csrf=${CSRF_NEW}" \
	"${BASEURL}/public/login.jsf" -D - > "$OUTPUT_FILE" 2>&1
