...
public class ValidationEngine {
	 ...
	 * @throws Exception
	 */
	public static boolean validateConfig(Config config)  throws Exception{
		...
		try {
						...
						if(itemBean.getSaveProperty()!= null && itemBean.getSaveProperty().isAppend()) {
							if(separatore == null) {
								...
								if(!separatore.equals(itemBean.getSaveProperty().getAppendSeparator()))
									throw new Exception("I separatori di append per la property ["+nomeProperty+"] devono essere tutti uguali.");
							}
						}
					}
				}
			...
			throw new Exception("Errore durante la validazione della configurazione ["+config.getName()+"]: "+ e.getMessage());
		}
		
		return true;
	}

	private static void validateSection(Section section,ConfigBean metadata) throws Exception{
		try {
			...
			throw new Exception("Section ["+section.getLabel()+"] -> "+ e.getMessage());
		}
	}

	private static void validaSubsection(Subsection subSection,ConfigBean metadata) throws Exception {
		try {
			...
			throw new Exception("Subsection ["+subSection.getLabel()+"] -> "+ e.getMessage());
		}
	}

	private static void validaItem(Item item,ConfigBean metadata) throws Exception {
		try {
			...
			if(item.getProperty().getProperties() != null) {
				if(!metadata.getListaNomiProperties().contains(item.getProperty().getProperties()))
					throw new Exception("Il nome delle properties ["+item.getProperty().getProperties()+"] indicato nella collection non e' dichiarato nella sezione collection della configurazione");
			}
			...
			throw new Exception("Item ["+item.getName()+"] non valido: "+ e.getMessage());
		}
	}

	private static void validaCheckBox(Item item) throws Exception{
		...
		if(property.isAppend()) {
			if(property.getSelectedValue().contains(property.getAppendSeparator()))
				throw new Exception("Il valore indicato per l'attributo SelectedValue ["+property.getSelectedValue()+"] contiene il separatore previsto per il salvataggio ["+property.getAppendSeparator()+"]");
			
			if(property.getUnselectedValue().contains(property.getAppendSeparator()))
				throw new Exception("Il valore indicato per l'attributo UnselectedValue ["+property.getUnselectedValue()+"] contiene il separatore previsto per il salvataggio ["+property.getAppendSeparator()+"]");
		}
		
	}

	private static void validaHidden(Item item) throws Exception{
		Property property = item.getProperty();
		// se e' una property di tipo append valido il separatore
		if(property.isAppend()) {
			if(item.getValue().contains(property.getAppendSeparator()))
				throw new Exception("Il valore indicato per l'attributo Value ["+item.getValue()+"] contiene il separatore previsto per il salvataggio ["+property.getAppendSeparator()+"]");
		}
	}

	private static void validaNumber(Item item) throws Exception{
		Property property = item.getProperty();
		// se e' una property di tipo append valido il separatore
		if(property.isAppend()) {
			if(item.getDefault().contains(property.getAppendSeparator()))
				throw new Exception("Il valore indicato per l'attributo Default ["+item.getValue()+"] contiene il separatore previsto per il salvataggio ["+property.getAppendSeparator()+"]");
		}
	}

	private static void validaSelect(Item item) throws Exception{
		Property property = item.getProperty();
		// se e' una property di tipo append valido il separatore
		if(property.isAppend()) {
			if(item.getDefault().contains(property.getAppendSeparator()))
		...
		if(item.getValues() == null || item.getValues().sizeValueList() == 0) {
			throw new Exception("E' necessario indicare una lista di Values per gli Item di tipo Select");
		}
	}

	private static void validaText(Item item) throws Exception{
		Property property = item.getProperty();
		// se e' una property di tipo append valido il separatore
		if(property.isAppend()) {
			if(item.getDefault().contains(property.getAppendSeparator()))
	...
	private static void validaConditions(Conditions conditions,ConfigBean metadata) throws Exception {
	...
	private static void validaCondition(Condition condition, int indice,ConfigBean metadata) throws Exception{ 
		if((condition.getDefinedList() == null || condition.getDefinedList().size() == 0) 
			...
			throw new Exception("La condition numero ["+indice+"] non e' valida: indicare almeno un elemento tra Defined, Equals o Selected.");
		...
		for (int i = 0; i < condition.getDefinedList().size(); i++) {
			...
			if(!itemDefined(defined.getName(),metadata))
				throw new Exception("L'elemento Defined numero ["+(i+1)+"] della Condition numero ["
		...
		for (int i = 0; i < condition.getEqualsList().size(); i++) {
			...
			if(!itemDefined(equals.getName(),metadata))
				throw new Exception("L'elemento Equals numero ["+(i+1)+"] della Condition numero ["
		...
		for (int i = 0; i < condition.getSelectedList().size(); i++) {
			...
			if(!itemDefined(selected.getName(),metadata))
				throw new Exception("L'elemento Selected numero ["+(i+1)+"] della Condition numero ["
	...
	private static void getMetadataSection(Section section, String sectionIdx,ConfigBean cbTmp) throws Exception{
	...
	private static void getMetadataSubsection(Subsection subSection, String subsectionIdx, ConfigBean cbTmp) throws Exception {
