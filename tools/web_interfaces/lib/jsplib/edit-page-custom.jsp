	...
	for (int z = 1; z < datiConGruppi.size(); z++) {
		...
		for (int i = 0; i < gruppo.size(); i++) {
	    			...
	    			if (type.equals("subtitle")){
			    				...
			    				<div class="box <%= labelStyleClass %>">
			    					<div class="boxTitle">	
			    						<% if(image != null && !image.equals("")){%>
			    							<span class="boxTitleIcon"><img src="images/tema_link/<%= image %>" title="<%= deLabel %>"/>&nbsp;</span>
			    						<% } %>
	        			...
	        			if (type.equals("text")){
                			...
                			if (type.equals("button")){
        		    			...
        		    			String classLink = "";
						  		String deTip =  " title=\"" + de.getValue() + "\"";
                				%>
                				<a class="<%= classLink %>" <%= deTip %> href="<%= de.getUrl() %>" type="button"><%= de.getValue() %></a>
                    			<%
                    		} else { // else button
                    			//fineelementi
                    		} // end else button
			...
			if(buttonGroupOpen){ // se c'e' un group aperto ed ho finito gli elementi devo chiudere
  			...
  			if(groupOpen){ // se c'e' un group aperto ed ho finito gli elementi devo chiudere
  				%>
  					</div>
  				</div>
  				<div class="addBox">
