...
public class ToggleStatoPortaService {
	...
	private static final String DISABILITATO = "disabilitato";
	
	private static final String MESSAGE_AUDITING_NON_EFFETTUATO = "Auditing dell'operazione [{}] non effettuato: {}";
	private static final String MESSAGE_AUDITING_NON_RIUSCITO = "Auditing dell'operazione [{}] non riuscito: {}";
	...
	public void toggleStatoPorta(String nomePorta, PddRuolo ruolo, boolean enable, String utente) throws ServiceException {
		...
		try {
			...
			logError("porta non trovata", nomePorta, e);
			...
			logError("errore DB per porta", nomePorta, e);
			registraErrore(idOperazioneAudit, utente, oggettoPortaAudit, enable, nomeGruppo, descrizioneGruppo, isDefaultGruppo, e.getMessage());
			throw new ServiceException("Errore durante il toggle dello stato della porta '" + nomePorta + "': " + e.getMessage(), e);
		} catch(Exception e) {
			logError("errore per porta", nomePorta, e);
			registraErrore(idOperazioneAudit, utente, oggettoPortaAudit, enable, nomeGruppo, descrizioneGruppo, isDefaultGruppo, e.getMessage());
			throw new ServiceException("Errore durante il toggle dello stato della porta '" + nomePorta + "': " + e.getMessage(), e);
		}
	}
	
	private void logError(String motivo, String nomePorta, Exception e) {
		this.log.error("toggleStatoPorta: {} [{}]: {}", motivo, nomePorta, e.getMessage(), e);
	}
	...
	private IDOperazione registraInElaborazione(String utente, Object oggetto, boolean enable,
		...
		try {
			...
			this.log.debug(MESSAGE_AUDITING_NON_EFFETTUATO, msg, disabilitato.getMessage());
			return null;
		} catch(Exception e) {
			this.log.error(MESSAGE_AUDITING_NON_RIUSCITO, msg, e.getMessage(), e);
			return null;
		}
	}
