...
public class ConnettoreStatusParams {	
	...
	private static final List<String> possibleResponseType = List.of("ModI", "Personalizzato");
	private static final List<String> possiblePersonalizedResponseType = List.of("vuoto", "xml", "json", "text");
	...
	protected void fillFrom(ConsoleHelper helper, Map<String, String> map, ServiceBinding serviceBinding, PageData pd) throws DriverControlStationException {
		...
		if (responseType != null && !possibleResponseType.contains(responseType))
			sendError(pd, ConnettoriCostanti.LABEL_CONNETTORE_RESPONSE_INPUT + ", valore non corretto, possibli valori: " + String.join(",", possibleResponseType));
		if (personalizedType != null && !possiblePersonalizedResponseType.contains(personalizedType))
			sendError(pd, ConnettoriCostanti.LABEL_CONNETTORE_RESPONSE_INPUT + ", valore non corretto, possibli valori: " + String.join(",", possiblePersonalizedResponseType));
		
		String type = (responseType == null || responseType.equals("ModI")) ? responseType : personalizedType;
		
		if (serviceBinding != null && serviceBinding.equals(ServiceBinding.SOAP) && type != null && !type.equals("ModI"))
		...
		if (ServletUtils.isCheckBoxEnabled(testStatisticsRaw)) {
			...
			if (periodRaw != null && !possiblePeriods.contains(periodRaw))
				sendError(pd, ConnettoriCostanti.LABEL_CONNETTORE_STATUS_PERIOD + ", valore non corretto, possibli valori: " + String.join(",", possiblePeriods));
	...
	public List<DataElement> getDati(List<DataElement> dati, ServiceBinding serviceBinding, boolean postBackViaPost) {
		...
		responseTypeSelect.setValues(possibleResponseType);		
		...
		personalizedResponseSelect.setValues(possiblePersonalizedResponseType);
		personalizedResponseSelect.setSelected(possiblePersonalizedResponseType.get(0));
		personalizedResponseSelect.setValue(possiblePersonalizedResponseType.get(0));
		...
		dInfo.setHeaderBody("valore ridefinito per il tipo della risposta");
		dInfo.setListBody(possiblePersonalizedResponseType);
		personalizedResponseSelect.setInfo(dInfo);
		
		if (getStatusResponseType() != null && !getStatusResponseType().equals("ModI")) {
			...
			responseTypeSelect.setSelected("Personalizzato");
		} else {
			personalizedResponseSelect.setType(DataElementType.HIDDEN);
			responseTypeSelect.setSelected("ModI");
		}
		...
		if (serviceBinding != null && serviceBinding.equals(ServiceBinding.SOAP)) {
			...
			responseTypeSelect.setSelected("ModI");
			responseTypeSelect.setValue("ModI");
		}
		
		// elementi per impostare la verifica connettivita
		DataElement connectivityCheckbox = new DataElement();
		connectivityCheckbox.setLabel(ConnettoriCostanti.LABEL_CONNETTORE_STATUS_TEST_CONNECTIVITY);
		...
		dInfo.setBody("abilita la verifica connettivita, la quale controlla che ogni connettore http(s) della porta applicativa/delegata sia raggiungibile");
		connectivityCheckbox.setInfo(dInfo);

		
		// elementi per impostare la verifica statistica
		DataElement statCheckbox = new DataElement();
		statCheckbox.setLabel(ConnettoriCostanti.LABEL_CONNETTORE_STATUS_TEST_STATISTICS);
		...
		dInfo.setBody("abilita la verifica statistica, la quale controlla che le esiste almeno una transazione che si riferisce alla stessa porta applicativa/delegata sia andata a buon fine nel caso ce ne siano");
		...
		dInfo.setBody("finestra temporale in cui effettuare la verifica statistica, vengono prese in considerazione transazioni effettuate solo all'interno dell'intervallo");
		...
		periodElement.setValue(Objects.requireNonNullElse(getPeriod(), possiblePeriods.get(0)));
		...
		dInfo.setBody("valore opzionale che indica il lifetime in cache delle analisi gia effettuate. "
				+ "NOTA: il valore deve essere minore o uguale al valore di default della cache controllo"
				+ " traffico/analisi statistica nel caso fosse superiore il valore di default verra impostato");
