 ...
 * 
 */
public final class ConfigurazioneTracciamentoAppenderChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (ServletUtils.isEditModeInProgress(request)) {
				...
				dati = confHelper.addTipoTracciamentoAppenderToDati(TipoOperazione.CHANGE, oa.getTipo(), dati, oa.getId() + "", oa.sizePropertyList());
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping,
						ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_TRACCIAMENTO_APPENDER, 
						ForwardParams.CHANGE());
			}

			// Controlli sui campi immessi
			boolean isOk = confHelper.tracciamentoAppenderCheckData(TipoOperazione.CHANGE);
			if (!isOk) {
				// setto la barra del titolo
				List<Parameter> lstParam = new ArrayList<Parameter>();
				...
				dati = confHelper.addTipoTracciamentoAppenderToDati(TipoOperazione.CHANGE, tipo,
			...
			return ServletUtils.getStrutsForwardError(ControlStationCore.getLog(), e, pd, session, gd, mapping, 
					ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_TRACCIAMENTO_APPENDER, ForwardParams.CHANGE());
		}
	}
}
