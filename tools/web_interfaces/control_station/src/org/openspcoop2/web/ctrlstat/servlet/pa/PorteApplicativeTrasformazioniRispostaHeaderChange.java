...
public class PorteApplicativeTrasformazioniRispostaHeaderChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String idTrasformazioneRispostaHeaderS = porteApplicativeHelper.getParameter(PorteApplicativeCostanti.PARAMETRO_PORTE_APPLICATIVE_ID_TRASFORMAZIONE_RISPOSTA_HEADER);
			...
			if (porteApplicativeHelper.isEditModeInProgress()) {
				...
				dati = porteApplicativeHelper.addTrasformazioneRispostaHeaderToDati(TipoOperazione.CHANGE, dati, idTrasformazioneS, idTrasformazioneRispostaS, idTrasformazioneRispostaHeaderS, nome, tipo, valore, identificazione, apc.getServiceBinding());
			...
			if (isOk && (oldParametro!=null && !oldParametro.getNome().equals(nome))) {
				...
				if (giaRegistrato) {
					pd.setMessage( ServletUtils.getMessageFromResourceBundle( session, "ControlStation.TrasformazioniCheckUniqueNomeTipoHeader" ) );	//CostantiControlStation.MESSAGGIO_TRASFORMAZIONI_CHECK_UNIQUE_NOME_TIPO_HEADER
					isOk = false;
				}
			} 
			if (!isOk) {
