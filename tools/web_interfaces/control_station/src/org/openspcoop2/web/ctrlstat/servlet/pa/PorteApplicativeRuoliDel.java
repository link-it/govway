 ...
 * 
 */
public final class PorteApplicativeRuoliDel extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			for (int i = 0; i < idsToRemove.size(); i++) {
				...
				if(isToken) {
					if(pa.getAutorizzazioneToken()!=null && pa.getAutorizzazioneToken().getRuoli()!=null) {
						for (int j = 0; j < pa.getAutorizzazioneToken().getRuoli().sizeRuoloList(); j++) {
							if(pa.getAutorizzazioneToken().getRuoli().getRuolo(j).getNome().equals(ruolo)){
								pa.getAutorizzazioneToken().getRuoli().removeRuolo(j);
								break;
							}
						}
					}
					...
					for (int j = 0; j < pa.getRuoli().sizeRuoloList(); j++) {
						if(pa.getRuoli().getRuolo(j).getNome().equals(ruolo)){
							pa.getRuoli().removeRuolo(j);
							break;
						}
					}
				}
			...
			porteApplicativeHelper.preparePorteApplicativeRuoliList(pa.getNome(), ricerca, lista);

			ServletUtils.setGeneralAndPageDataIntoSession(request, session, gd, pd);
			// Forward control to the specified success URI
			return ServletUtils.getStrutsForward (mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_RUOLI, 
					ForwardParams.DEL());
		} catch (Exception e) {
