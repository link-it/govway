 ...
 * 
 */
public final class PorteDelegateProprietaAutorizzazioneContenutoChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (porteDelegateHelper.isEditModeInProgress()) {
				...
				if(valore==null){
					for (int i = 0; i < pde.sizeProprietaAutorizzazioneContenutoList(); i++) {
						Proprieta ssp = pde.getProprietaAutorizzazioneContenuto(i);
			...
			boolean isOk = porteDelegateHelper.proprietaAutorizzazioneContenutoCheckData(TipoOperazione.CHANGE, idPorta, nome, valore);
			if (!isOk) {
				// setto la barra del titolo
				ServletUtils.setPageDataTitle(pd, lstParam);
			...
			for (int i = 0; i < pde.sizeProprietaAutorizzazioneContenutoList(); i++) {
				Proprieta ssp = pde.getProprietaAutorizzazioneContenuto(i);
				if (nome.equals(ssp.getNome())) {
					pde.removeProprietaAutorizzazioneContenuto(i);
					break;
				}
			}
