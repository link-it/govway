...
public class UtentiHelper extends ConsoleHelper {
	...
	public List<DataElement> addUtentiToDati(List<DataElement> dati,TipoOperazione tipoOperazione,boolean singlePdD,
		...
		if(utenteConsoleEnabled || utenteMonitorEnabled) {
			...
			if(utenteConsoleEnabled) {
				...
				// "Modalita' Interfaccia" e' specifico della Console e va in coda alla sotto-sezione,
				// dopo "Profilo Interoperabilita'" e "Soggetto Operativo", in modo che entrambe le sotto-sezioni
				// (govwayConsole e govwayMonitor) condividano lo stesso ordine iniziale dei campi.
				de = new DataElement();
			...
			if(utenteMonitorEnabled) {
				...
				if(protocolloSelezionato) {
					...
					if((profiloDefaultConsoleMonitoraggio!=null && !UtentiCostanti.VALORE_PARAMETRO_MODALITA_ALL.equals(profiloDefaultConsoleMonitoraggio)) ||
						...
						// Se l'utente non e' abilitato a "Tutti" i soggetti per il monitor,
						// restringo la tendina ai soli soggetti su cui l'utente ha visibilita'.
						boolean permitAllSoggettiMonitor = ServletUtils.isCheckBoxEnabled(isSoggettiAll);
						if(idSoggettiOperativi != null && !permitAllSoggettiMonitor && oldImgUser != null) {
							List<IDSoggetto> soggettiAutorizzati = oldImgUser.getSoggetti();
							if(soggettiAutorizzati == null || soggettiAutorizzati.isEmpty()) {
								idSoggettiOperativi = new ArrayList<>();
							} else {
								List<IDSoggetto> filtrati = new ArrayList<>();
								for (IDSoggetto idOp : idSoggettiOperativi) {
									for (IDSoggetto idAuth : soggettiAutorizzati) {
										if(idOp.getTipo()!=null && idOp.getTipo().equals(idAuth.getTipo())
												&& idOp.getNome()!=null && idOp.getNome().equals(idAuth.getNome())) {
											filtrati.add(idOp);
											break;
										}
									}
								}
								idSoggettiOperativi = filtrati;
							}
						}
						...
						if(idSoggettiOperativi != null && idSoggettiOperativi.size()>1) {
							...
							if(idSoggettiOperativi!=null && !idSoggettiOperativi.isEmpty()) {
								IDSoggetto idSoggetto = idSoggettiOperativi.get(0);
								String labelSoggetto = ConsoleHelper._getLabelNomeSoggetto(idSoggetto);
								de.setValue(labelSoggetto);
							}
	...
	public void prepareUtentiList(ISearch ricerca, List<User> lista, boolean singlePdD) throws Exception {
		try {
			...
			labels.add("");
			labels.add(UtentiCostanti.LABEL_UTENTE);
			if(this.utentiCore.isMultiLoginEnabled()) {
				labels.add(UtentiCostanti.LABEL_INFO_UTENTE_TIPOLOGIA);
			}
			labels.add(UtentiCostanti.LABEL_MODALITA_INTERFACCIA);
			labels.add(UtentiCostanti.LABEL_MODALITA_GATEWAY_COMPACT);
			labels.add(UtentiCostanti.LABEL_PERMESSI_GESTIONE);
			labels.add(UtentiCostanti.LABEL_CAMBIA_IDENTITA);
			...
			if (lista != null) {
				...
				while (it.hasNext()) {
					...
					if(this.core.isCheckPasswordExpire(passwordVerifier) &&
						mySU.isCheckLastUpdatePassword() &&
						passwordVerifier.isPasswordExpire(mySU.getLastUpdatePassword())) {
					...
					if(this.utentiCore.isMultiLoginEnabled()) {
						de = new DataElement();
						boolean utenteEsterno = this.utentiCore.getUtenzePasswordManager().check(UtentiCostanti.VALORE_PARAMETRO_PW_MODALITA_NO_LOGIN_APPLICATION, mySU.getPassword());
						de.setValue( utenteEsterno ? UtentiCostanti.LABEL_INFO_UTENTE_ESTERNO : UtentiCostanti.LABEL_INFO_UTENTE_APPLICATIVO);
						e.add(de);
					}
