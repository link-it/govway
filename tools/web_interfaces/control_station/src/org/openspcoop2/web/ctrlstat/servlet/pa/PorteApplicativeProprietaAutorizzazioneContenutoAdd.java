...
/**
 * PorteApplicativeProprietaAutorizzazioneContenutoAdd
 * 
 ...
 * 
 */
public final class PorteApplicativeProprietaAutorizzazioneContenutoAdd extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (porteApplicativeHelper.isEditModeInProgress()) {
				...
				dati = porteApplicativeHelper.addProprietaAutorizzazioneCustomToDati(dati, TipoOperazione.ADD, nome, valore);
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_AUTORIZZAZIONE_CONTENUTI_CUSTOM_PROPERTIES,
						ForwardParams.ADD());
			}

			// Controlli sui campi immessi
			boolean isOk = porteApplicativeHelper.proprietaAutorizzazioneContenutoCheckData(TipoOperazione.ADD, idPorta, nome, valore);
			if (!isOk) {
				...
				return ServletUtils.getStrutsForwardEditModeCheckError(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_AUTORIZZAZIONE_CONTENUTI_CUSTOM_PROPERTIES, 
						ForwardParams.ADD());
			}
			...
			List<Proprieta> lista = porteApplicativeCore.porteApplicativeAutorizzazioneContenutoCustomPropList(idInt, ricerca);

			porteApplicativeHelper.preparePorteApplicativeAutorizzazioneContenutoCustomPropList(ricerca, lista);

			ServletUtils.setGeneralAndPageDataIntoSession(request, session, gd, pd);
			// Forward control to the specified success URI
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_AUTORIZZAZIONE_CONTENUTI_CUSTOM_PROPERTIES, 
					ForwardParams.ADD());
		} catch (Exception e) {
