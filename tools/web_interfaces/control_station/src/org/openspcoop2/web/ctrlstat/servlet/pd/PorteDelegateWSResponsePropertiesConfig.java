...
/**     
 * PorteDelegateWSResponsePropertiesConfig
 *
 * @author Pintori Giuliano (pintori@link.it)
 * @author $Author$
 * @version $Rev$, $Date$
 */
public class PorteDelegateWSResponsePropertiesConfig  extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			Map<String, Properties> mappaDB = porteDelegateCore.readMessageSecurityResponsePropertiesConfiguration(pde.getId()); 
			
			ConfigBean configurazioneBean = porteDelegateCore.leggiConfigurazione(configurazione, mappaDB);
			...
			lstParam.add(new Parameter(PorteDelegateCostanti.LABEL_PARAMETRO_PORTE_DELEGATE_MESSAGE_SECURITY_RESPONSE_FLOW_PROPERTIES_CONFIG + " " + configurazione.getLabel() ,null));
...
//				configurazioneBean.updateConfigurazione(configurazione);
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_MESSAGE_SECURITY_RESPONSE_PROPERTIES_CONFIG, ForwardParams.OTHER(""));
			}
...
//				configurazioneBean.updateConfigurazione(configurazione);
				...
				return ServletUtils.getStrutsForwardEditModeCheckError(mapping,	PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_MESSAGE_SECURITY_RESPONSE_PROPERTIES_CONFIG, ForwardParams.OTHER(""));
			}
			...
			if(pde.getMessageSecurity().getResponseFlow().getParameterList() == null)
				pde.getMessageSecurity().getResponseFlow().setParameterList(new ArrayList<MessageSecurityFlowParameter>());
			...
			for (String propKey : map.keySet()) {
				...
				pde.getMessageSecurity().getResponseFlow().addParameter(parameter);
			}
			...
			mappaDB = porteDelegateCore.readMessageSecurityResponsePropertiesConfiguration(pde.getId()); 
			
			configurazioneBean = porteDelegateCore.leggiConfigurazione(configurazione, mappaDB);
			...
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_MESSAGE_SECURITY_RESPONSE_PROPERTIES_CONFIG, ForwardParams.OTHER(""));
		} catch (Exception e) {
			return ServletUtils.getStrutsForwardError(ControlStationCore.getLog(), e, pd, request, session, gd, mapping, 
					PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_MESSAGE_SECURITY_RESPONSE_PROPERTIES_CONFIG, 
					ForwardParams.OTHER(""));
		}  
	}
}
