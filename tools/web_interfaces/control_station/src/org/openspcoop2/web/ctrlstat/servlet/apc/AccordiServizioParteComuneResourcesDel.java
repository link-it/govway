...
//				try{
//					idServizi = apsCore.getIdServiziWithPortType(idRisorsa);
...
//						if(pt.getNome().equals(nomeRisorsa)==false){
//							for (int check = 0; check < pt.sizeAzioneList(); check++) {
//								Operation opCheck = pt.getAzione(check);
//								if(opCheck.getCorrelataServizio()!=null && nomeRisorsa.equals(opCheck.getCorrelataServizio())){
...
//					}
//					if(tmp.size()>0){
//						errori.append("Servizio ["+nomeRisorsa+"] non rimosso perche' correlato da azioni di altri servizi dell'accordo: <br>");
...
//					for (int j = 0; j < as.sizePortTypeList(); j++) {
//						PortType pt = as.getPortType(j);
//						if (nomeRisorsa.equals(pt.getNome())) {
...
//				}else{
//					
//					errori.append("Servizio ["+nomeRisorsa+"] non rimosso perche' utilizzato in accordi di servizio parte specifica: <br>");
