...
/**
 * PorteApplicativeProprietaAutorizzazioneAdd
 * 
 ...
 * 
 */
public final class PorteApplicativeProprietaAutorizzazioneChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (porteApplicativeHelper.isEditModeInProgress()) {
				...
				if(valore==null){
					for (int i = 0; i < pa.sizeProprietaAutorizzazioneList(); i++) {
						Proprieta ssp = pa.getProprietaAutorizzazione(i);
				...
				dati = porteApplicativeHelper.addProprietaAutorizzazioneCustomToDati(dati, TipoOperazione.CHANGE, nome, valore);
			...
			boolean isOk = porteApplicativeHelper.proprietaAutorizzazioneCheckData(TipoOperazione.CHANGE, idPorta, nome, valore);
			if (!isOk) {
			...
			for (int i = 0; i < pa.sizeProprietaAutorizzazioneList(); i++) {
				Proprieta ssp = pa.getProprietaAutorizzazione(i);
				if (nome.equals(ssp.getNome())) {
					pa.removeProprietaAutorizzazione(i);
					break;
				}
			}
