...
/**     
 * ConfigurazioneHandlersRichiestaAdd
 *
 * @author Pintori Giuliano (pintori@link.it)
 * @author $Author$
 * @version $Rev$, $Date$
 */
public class ConfigurazioneHandlersRichiestaAdd extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			List<ConfigurazioneHandlerBean> listaEsistenti = confCore.handlersRichiestaList(ricercaPluginUilizzati, fase, ruoloPorta, idPorta); 
			...
			String messaggioHandlerRichiestaDuplicato = null;
			String tipo = null;
			Plugin plugin = null; 
			...
			if(plugin != null) {
				messaggioHandlerRichiestaDuplicato = MessageFormat.format(ConfigurazioneCostanti.MESSAGGIO_HANDLER_RICHIESTA_DUPLICATO, labelHandler, plugin.getLabel());
				tipo = plugin.getTipo();
			}
			...
			if (confHelper.isEditModeInProgress()) {
				...
				confHelper.addHandlerRichiestaToDati(dati, tipoOperazione, null, nomePlugin, stato, ruoloPorta, idPortaS, serviceBinding, fase, tipiPluginGiaUtilizzati, messaggioValoriNonDisponibili); 
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_HANDLERS_RICHIESTA, ForwardParams.ADD());
			}
			
			// Controlli sui campi immessi
			boolean isOk = confHelper.handlerRichiestaCheckData(TipoOperazione.ADD, null, nomePlugin, stato, ruoloPorta, idPorta, fase, tipo, messaggioHandlerRichiestaDuplicato);
			if (!isOk) {
				ServletUtils.setPageDataTitle(pd, lstParam);
			...
			int posizione = confCore.getMaxPosizioneHandlersRichiesta(fase,ruoloPorta,idPorta) + 1;
			...
			if(ruoloPorta !=null) {
				if(ruoloPorta.equals(TipoPdD.DELEGATA)) {
					...
					switch (faseMH) {
					case IN:
						portaDelegata.getConfigurazioneHandler().getRequest().addIn(handler);
						break;
					case IN_PROTOCOL_INFO:
						portaDelegata.getConfigurazioneHandler().getRequest().addInProtocolInfo(handler);
						break;
					case OUT:
						portaDelegata.getConfigurazioneHandler().getRequest().addOut(handler);
						break;
					case POST_OUT:
						portaDelegata.getConfigurazioneHandler().getRequest().addPostOut(handler);
						break;
					case PRE_IN:
						portaDelegata.getConfigurazioneHandler().getRequest().addPreIn(handler);
						break;
					}
					...
					switch (faseMH) {
					case IN:
						portaApplicativa.getConfigurazioneHandler().getRequest().addIn(handler);
						break;
					case IN_PROTOCOL_INFO:
						portaApplicativa.getConfigurazioneHandler().getRequest().addInProtocolInfo(handler);
						break;
					case OUT:
						portaApplicativa.getConfigurazioneHandler().getRequest().addOut(handler);
						break;
					case POST_OUT:
						portaApplicativa.getConfigurazioneHandler().getRequest().addPostOut(handler);
						break;
					case PRE_IN:
						portaApplicativa.getConfigurazioneHandler().getRequest().addPreIn(handler);
						break;
					}
				...
				switch (faseMH) {
				case IN:
					configurazione.getConfigurazioneHandler().getRequest().addIn(handler);
					break;
				case IN_PROTOCOL_INFO:
					configurazione.getConfigurazioneHandler().getRequest().addInProtocolInfo(handler);
					break;
				case OUT:
					configurazione.getConfigurazioneHandler().getRequest().addOut(handler);
					break;
				case POST_OUT:
					configurazione.getConfigurazioneHandler().getRequest().addPostOut(handler);
					break;
				case PRE_IN:
					configurazione.getConfigurazioneHandler().getRequest().addPreIn(handler);
					break;
				}
			...
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_HANDLERS_RICHIESTA, ForwardParams.ADD());
		} catch (Exception e) {
			return ServletUtils.getStrutsForwardError(ControlStationCore.getLog(), e, pd, session, gd, mapping, 
					ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_HANDLERS_RICHIESTA, ForwardParams.ADD());
		}  
	}
}			
