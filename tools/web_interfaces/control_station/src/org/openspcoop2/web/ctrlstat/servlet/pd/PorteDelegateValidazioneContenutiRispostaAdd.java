...
/**
 * PorteDelegateValidazioneContenutiRispostaAdd
 * 
 ...
 * 
 */
public class PorteDelegateValidazioneContenutiRispostaAdd extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (porteDelegateHelper.isEditModeInProgress()) {
				...
				dati = porteDelegateHelper.addValidazioneContenutiRispostaToDati(dati, TipoOperazione.ADD, null, 
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_RISPOSTA, ForwardParams.ADD());
			}
			
			boolean isOk = porteDelegateHelper.validazioneContenutiRispostaCheckData(TipoOperazione.ADD, null, true, portaDelegata.getId());
			if (!isOk) {
				...
				return ServletUtils.getStrutsForwardEditModeCheckError(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_RISPOSTA,	ForwardParams.ADD());
			}
			
			// calcolo prossima posizione
			int posizione = 1;
			for (ValidazioneContenutiApplicativiRisposta check : portaDelegata.getValidazioneContenutiApplicativi().getRispostaList()) {
			...
			portaDelegata.getValidazioneContenutiApplicativi().addRisposta(risposta);
			...
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_RISPOSTA, ForwardParams.ADD());


		} catch (Exception e) {
			return ServletUtils.getStrutsForwardError(ControlStationCore.getLog(), e, pd, session, gd, mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_RISPOSTA, ForwardParams.ADD());
		} 
	}
}
