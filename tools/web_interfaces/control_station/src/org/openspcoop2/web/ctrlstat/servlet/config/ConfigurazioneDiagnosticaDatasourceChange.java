 ...
 * 
 */
public final class ConfigurazioneDiagnosticaDatasourceChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (ServletUtils.isEditModeInProgress(request)) {
				...
				dati = confHelper.addDiagnosticaDatasourceToDati(TipoOperazione.CHANGE, od.getNome(), od.getNomeJndi(),
			...
			boolean isOk = confHelper.diagnosticaDatasourceCheckData(TipoOperazione.CHANGE);
			if (!isOk) {
				// setto la barra del titolo
				List<Parameter> lstParam = new ArrayList<Parameter>();
			...
			for (int i = 0; i < md.sizeOpenspcoopSorgenteDatiList(); i++) {
				OpenspcoopSorgenteDati tmpOd = lista.get(i);
				if (idInt == tmpOd.getId().intValue()) {
					md.removeOpenspcoopSorgenteDati(i);
					break;
				}
			}
			...
			md.addOpenspcoopSorgenteDati(newOd);
			...
			return ServletUtils.getStrutsForwardError(ControlStationCore.getLog(), e, pd, session, gd, mapping, 
					ConfigurazioneCostanti.OBJECT_NAME_CONFIGURAZIONE_DIAGNOSTICA_DATASOURCE, ForwardParams.CHANGE());
		}
	}
}
