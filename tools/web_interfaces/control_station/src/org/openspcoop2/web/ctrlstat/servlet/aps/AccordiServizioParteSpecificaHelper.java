...
//					AccordiServizioParteSpecificaCostanti.SERVLET_NAME_APS_LIST));
			...
			for (int i = 0; i < lista.size(); i++) {
				...
				if(showConfigurazione) {
					...
					if(!isPddEsterna){
						IDPortaApplicativa idPA = this.porteApplicativeCore.getIDPortaApplicativaAssociataDefault(idServizio);
						PortaApplicativa paDefault = this.porteApplicativeCore.getPortaApplicativa(idPA);
	...
	public void serviziFruitoriMappingList(List<MappingFruizionePortaDelegata> lista, String idServizio, String idSoggettoFruitore, String idFruzione, ISearch ricerca)	throws Exception {
		try {
			...
			if (search.equals("")) {
				...
				lstParm.add(new Parameter(AccordiServizioParteSpecificaCostanti.LABEL_APS_CONFIGURAZIONI_DI + fruitoreTmpTile, AccordiServizioParteSpecificaCostanti.SERVLET_NAME_APS_FRUITORI_PORTE_DELEGATE_LIST, pIdFruitore,pIdServizio,pIdSoggettoFruitore));
			...
			//listaLabel.add(PorteDelegateCostanti.LABEL_PARAMETRO_PORTE_DELEGATE_CONFIGURAZIONE); // spostata direttamente nell'elenco delle fruizioni
