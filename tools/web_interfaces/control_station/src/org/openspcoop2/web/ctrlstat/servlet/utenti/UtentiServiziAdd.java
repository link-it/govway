			...
			if(utentiHelper.isEditModeInProgress()){
				...
				if(listaServiziNonUtilizzati.size()<=0){
					if(protocolli.size() == 1) {
						pd.setMessage( ServletUtils.getMessageFromResourceBundle( session, "Utenti.UtentiServiziDisponibiliEsauriti" ) , MessageType.INFO);	//UtentiCostanti.LABEL_UTENTI_SERVIZI_DISPONIBILI_ESAURITI
						pd.disableEditMode();
						pd.setDati(dati);
