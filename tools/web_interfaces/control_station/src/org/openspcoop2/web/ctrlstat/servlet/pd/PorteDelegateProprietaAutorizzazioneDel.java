...
/**
 * PorteDelegateProprietaAutorizzazioneDel
 * 
 ...
 * 
 */
public final class PorteDelegateProprietaAutorizzazioneDel extends Action {
		...
		try {
			...
			for (int i = 0; i < idsToRemove.size(); i++) {
				...
				for (int j = 0; j < pde.sizeProprietaAutorizzazioneList(); j++) {
					Proprieta ssp = pde.getProprietaAutorizzazione(j);
					if (idToRemove == ssp.getId().longValue()) {
						pde.removeProprietaAutorizzazione(j);
						break;
					}
				}
			}
			...
			porteDelegateHelper.preparePorteDelegateAutorizzazioneCustomPropList(pde.getNome(), ricerca, lista);
