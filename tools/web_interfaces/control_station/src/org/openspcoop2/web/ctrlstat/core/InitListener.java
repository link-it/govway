		...
		try {
			...
			try{
				if(consoleProperties.isSinglePdD()==null || (!consoleProperties.isSinglePdD())){
					InitListener.logInfo("Inizializzazione Gestori, della govwayConsole Centralizzata, in corso...");
				
	                this.gestoriStartupThread = new GestoriStartupThread();
	                new Thread(this.gestoriStartupThread).start();
					
					InitListener.logInfo("Inizializzazione Gestori, della govwayConsole Centralizzata, effettuata con successo.");
				}
			}catch(Exception e){
				throw new UtilsRuntimeException(e.getMessage(),e);
			}
