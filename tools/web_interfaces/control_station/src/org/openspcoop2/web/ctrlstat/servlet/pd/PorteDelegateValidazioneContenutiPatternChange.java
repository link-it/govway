...
/**
 * PorteDelegateValidazioneContenutiPatternChange
 * 
 ...
 * 
 */
public class PorteDelegateValidazioneContenutiPatternChange extends Action {
		...
		try {
			...
			if(tipoParent.equals(CostantiControlStation.DEFAULT_VALUE_PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_PATTERN_PARENT_RICHIESTA)) {
				...
				for (int j = 0; j < portaDelegata.getValidazioneContenutiApplicativi().getConfigurazione().getConfigurazionePattern().sizePatternList(); j++) {
					ValidazioneContenutiApplicativiPatternRegola pattern = portaDelegata.getValidazioneContenutiApplicativi().getConfigurazione().getConfigurazionePattern().getPattern(j);
					if (pattern.getId().longValue() == idRegola.longValue()) {
						oldPattern = pattern;
						break;
					}
			...
			if (porteDelegateHelper.isEditModeInProgress()) {
				...
				dati = porteDelegateHelper.addValidazioneContenutiPatternToDati(dati, TipoOperazione.CHANGE, oldPattern, idRegolaS, nome, regola, patternAndS, patternNotS);
				...
				dati = porteDelegateHelper.addHiddenFieldsPatternParentToDati(TipoOperazione.CHANGE, idParent, tipoParent, dati);
				
				pd.setDati(dati);

				ServletUtils.setGeneralAndPageDataIntoSession(session, gd, pd);

				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_PATTERN,	ForwardParams.CHANGE());
			}
			
			boolean isOk = porteDelegateHelper.validazioneContenutiPatternCheckData(TipoOperazione.CHANGE, oldPattern, true, portaDelegata.getId());
			if (!isOk) {
				...
				return ServletUtils.getStrutsForwardEditModeCheckError(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_PATTERN,	ForwardParams.CHANGE());
			}
			...
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, PorteDelegateCostanti.OBJECT_NAME_PORTE_DELEGATE_VALIDAZIONE_CONTENUTI_PATTERN,	ForwardParams.CHANGE());
		} catch (Exception e) {
