...
/**
 * PorteApplicativeValidazioneContenutiRichiestaChange
 * 
 ...
 * 
 */
public class PorteApplicativeValidazioneContenutiRichiestaChange extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			String nome = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_NOME);
			// configurazione stato (ereditata da validazionecontenuti)
			String statoValidazione = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_STATO);
			String tipoValidazione = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_TIPO);
			String applicaMTOM = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_MTOM);

			String soapAction = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_SOAP_ACTION); 
			String jsonSchema = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_JSON_SCHEMA);
			
			String patternAndS = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_AND);
			String patternNotS = porteApplicativeHelper.getParameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_NOT); 
			...
			for (int j = 0; j < pa.getValidazioneContenutiApplicativi().sizeRichiestaList(); j++) {
				...
				if (richiesta.getId().longValue() == idRichiesta.longValue()) {
					oldRichiesta = richiesta;
					break;
				}
			}
			
			ValidazioneContenutiApplicativiStato validazioneContenutiApplicativiStato = oldRichiesta != null ? oldRichiesta.getConfigurazione() : null; 
			ValidazioneContenutiApplicativiRichiestaApplicabilita validazioneContenutiApplicativiApplicabilita = oldRichiesta != null ? oldRichiesta.getApplicabilita() : null; 
			...
			Parameter pIdRichiesta = new Parameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_RICHIESTA_ID_RICHIESTA, idRichiestaS);
			Parameter pIdPatternParent = new Parameter(CostantiControlStation.PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_PATTERN_PARENT,
					CostantiControlStation.DEFAULT_VALUE_PARAMETRO_PORTE_VALIDAZIONE_CONTENUTI_PATTERN_PARENT_RICHIESTA);
			
			paramsPatternList.add(pIdPorta);
			paramsPatternList.add(pIdSoggetto);
			paramsPatternList.add(pIdAsps);
			paramsPatternList.add(pIdRichiesta);
			paramsPatternList.add(pIdPatternParent);
			List<String> listaJsonSchema = new ArrayList<String>();
			int numeroPattern = porteApplicativeCore.numeroPatternValidazioneContenutiRichiesta(idRichiesta);
			...
			lstParam.add(new Parameter(oldRichiesta.getNome(), null));
			...
			if (porteApplicativeHelper.isEditModeInProgress()) {
				...
				dati = porteApplicativeHelper.addValidazioneContenutiRichiestaToDati(dati, TipoOperazione.CHANGE, oldRichiesta.getId()+"", 
						idRichiestaS, nome, false, statoValidazione, tipoValidazione, applicaMTOM, 
				...
				return ServletUtils.getStrutsForwardEditModeInProgress(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_VALIDAZIONE_CONTENUTI_RICHIESTA,	ForwardParams.CHANGE());
			}
			
				
			boolean isOk = porteApplicativeHelper.validazioneContenutiRichiestaCheckData(TipoOperazione.CHANGE, oldRichiesta, false, pa.getId());
			if (!isOk) {
				...
				return ServletUtils.getStrutsForwardEditModeCheckError(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_VALIDAZIONE_CONTENUTI_RICHIESTA,	ForwardParams.CHANGE());
			}
			...
			return ServletUtils.getStrutsForwardEditModeFinished(mapping, PorteApplicativeCostanti.OBJECT_NAME_PORTE_APPLICATIVE_VALIDAZIONE_CONTENUTI_RICHIESTA,	ForwardParams.CHANGE());
		} catch (Exception e) {
