...
public class TestOptimizedHeader {
		...
		try {
			...
			if(testDimensioneKb==null || testDimensioneKb>0) {
				...
				for (Integer dimensione : dimensioni) {
					...
					if(dimensione.intValue()>2000) {
						// verifico solo i primi due test
						continue;
					}
	...
	private static InputStream getInputStream(String fileName, int kbBody, byte [] resource) throws Exception {
		...
		FileSystemUtilities.writeFile(fTmp, resource);
	...
	private static void test(String fileName, int kbBody, String contentType, MessageType messageType, int bufferThresholdKb, boolean attesoComeOttimizzabile) throws Exception {
		...
		long rapporto_left_interval = 0;
		
		long rapporto_min_saaj = 28;
		...
		long rapporto_min = 1;
		long rapporto_stream_reader = 1;
		
		long rapporto_min_after_header = 1;
		long rapporto_stream_reader_after_header = 1;
		
		long rapporto_write_to = 1;
		...
		if(fileName.contains("Header5K") && !fileName.contains("requestHeader5KBody5K")){
			...
			rapporto_stream_reader_after_header = 45;
			rapporto_write_to=2;
			...
			incremento_minimo_memoria = 10;
			rapporto_stream_reader = 3;
			rapporto_stream_reader_after_header = 10;
			...
			rapporto_stream_reader_after_header = 25;
			rapporto_min_saaj = 15;
		}
			...
			rapporto_max_dom = rapporto_max_dom*20; 
			rapporto_stream_reader = 10;
			rapporto_stream_reader_after_header = 30;
			rapporto_min_saaj = 20;
			rapporto_max_saaj = 50;
			tolleranza_bytes = 1024*3; // 3 kb
		}
		...
		for (int i = 0; i < iterazioniPossibili; i++) {
			...
			printMemory(i, iterazioniPossibili, tolleranza_bytes);
			
			try(InputStream isMessage = getInputStream(fileName, kbBody, resource)){
				...
				print(i, "StreamReader", streamReader, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min : rapporto_stream_reader);
				...
				print(i, "OpenSPCoop2Message-init", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min : rapporto_stream_reader);
				...
				long dimensione = print(i, "OpenSPCoop2Message-after-write", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min : rapporto_stream_reader);
				if(i==0) {
				...
				print(i, "finalResourceRaw", bout.toByteArray(), initial, rapporto_left_interval, rapporto_write_to);
			...
			try(InputStream isMessage = getInputStream(fileName, kbBody, resource)){
				...
				if(isOptimizable) {
					if(fileName.contains("Header5K") || fileName.startsWith("requestSuUnaRigaSoapBodyVuoto")) {
						print(i, "OpenSPCoop2Message-after-header", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min * 2l : rapporto_max_saaj); // viene costruito saaj in reader
					}
					else if(fileName.startsWith("requestSoapPrefixEmptyWithHeader") || fileName.startsWith("requestMixedPrefix")) {
						print(i, "OpenSPCoop2Message-after-header", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min * 4l : rapporto_min * 4l); // viene costruito saaj in reader
					}
					else {
						print(i, "OpenSPCoop2Message-after-header", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min_after_header : rapporto_stream_reader_after_header);
								//rapporto_min * 2l : rapporto_stream_reader+1);
					}
				}
				else {
				...
				if(isOptimizable) {
					...
					dimensione = print(i, "OpenSPCoop2Message-after-header-write", msg, initial, rapporto_left_interval, bufferThresholdKb==1 ? rapporto_min : rapporto_stream_reader); 
							//rapporto_min * 2l : rapporto_stream_reader+1);
				}
				else {
		...
		for (int i = 0; i < iterazioniPossibili; i++) {
			...
			try(InputStream isMessage = getInputStream(fileName, kbBody, resource)){
				...
				long dimensione = -1;
				try {
					dimensione = print(i, "OpenSPCoop2Message-after-write", msg, initial, rapporto_min_saaj, rapporto_max_saaj);
				}catch(Exception e) {
					excTooBig.add(e);
					System.out.println("Saved in list");
					if(soap.getSoapReader().getHeader_OpenSPCoop2Message()!=null) {
						print(i, "Verifica interna soap header nel reader", soap.getSoapReader().getHeader_OpenSPCoop2Message(), initial, 0l, 300l);
					}
					print(i, "Verifica interna soap reader", soap.getSoapReader(), initial, 0l, 300l);
					if(excTooBig.size()>MAX_EXC) {
						throw e;
					}
				}
				if(i==0) {
	...
	private static long print(int iterazione, String name, Object resource, Long initial, Long rapportoAttesoMin, Long rapportoAttesoMax) throws Exception {
		if(testHeap && iterazione==0) {			
			...
			if(initial!=null) {
				if(rapportoAttesoMax!=null && rapportoAttesoMax.longValue()==0l) {
