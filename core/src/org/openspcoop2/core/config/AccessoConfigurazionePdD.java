 ...
 * @version $Rev$, $Date$
 */
public class AccessoConfigurazionePdD implements Serializable , Cloneable{
	...
	private boolean utilizzoCache;
	/** dimensione cache */
	private Integer dimensioneCache;
	/** algoritmo */
	private AlgoritmoCache algoritmoCache;
	/** item idle time */
	private Integer itemIdleTimeCache;
	/** item life time */
	private Integer itemLifeTimeCache;
	...
	/**
	 * Imposta l'indicazione sull'utilizzo o meno di una cache.
	 *
	 * @param value Indicazione sull'utilizzo della cache
	 * 
	 */
	public void setUtilizzoCache(boolean value){
		this.utilizzoCache = value;
	}

	/**
	 * Imposta la dimensione della cache.
	 *
	 * @param dimesioneCache dimensione della cache
	 * 
	 */
	public void setDimensioneCache(Integer dimesioneCache) {
		this.dimensioneCache = dimesioneCache;
	}

	/**
	 * Imposta l'algoritmo della cache.
	 *
	 * @param value algoritmo della cache
	 * 
	 */
	public void setAlgoritmoCache(AlgoritmoCache value){
		this.algoritmoCache = value;
	}

	/**
	 * Imposta l'idle time degli elementi della cache.
	 *
	 * @param value idle time degli elementi
	 * 
	 */
	public void setItemIdleTimeCache(Integer value){
		this.itemIdleTimeCache = value;
	}

	/**
	 * Imposta il life-time degli elementi della cache.
	 *
	 * @param value life-time degli elementi
	 * 
	 */
	public void setItemLifeTimeCache(Integer value){
		this.itemLifeTimeCache = value;
	}
	...
	/**
	 * Ritorna l'indicazione sull'utilizzo o meno di una cache.
	 *
	 * @return Indicazione sull'utilizzo della cache
	 * 
	 */
	public boolean getUtilizzoCache(){
		return this.utilizzoCache;
	}

	/**
	 * Ritorna la dimensione della cache.
	 *
	 * @return dimensione della cache
	 * 
	 */
	public Integer getDimensioneCache() {
		return this.dimensioneCache;
	}

	/**
	 * Ritorna l'algoritmo della cache.
	 *
	 * @return algoritmo della cache
	 * 
	 */
	public AlgoritmoCache getAlgoritmoCache(){
		return this.algoritmoCache;
	}

	/**
	 * Ritorna l'idle time degli elementi della cache.
	 *
	 * @return idle time degli elementi
	 * 
	 */
	public Integer getItemIdleTimeCache(){
		return this.itemIdleTimeCache;
	}

	/**
	 * Ritorna il life-time degli elementi della cache.
	 *
	 * @return life-time degli elementi
	 * 
	 */
	public Integer getItemLifeTimeCache(){
		return this.itemLifeTimeCache;
	}
	...
	public boolean equals(Object object,boolean checkID){
		try{
			...
			for(int i=0; i<fields.length;i++){
				...
				if(checkID==false){
					if("id".equals(fields[i].getName()) || "idAccordo".equals(fields[i].getName()) || "idSoggetto".equals(fields[i].getName()))
						continue;
				}
				if(fields[i].get(this)==null){
					...
					if(fields[i].getType().getName().equals("java.util.ArrayList")){
						java.util.ArrayList<?> lista_this = (java.util.ArrayList<?>) fields[i].get(this);
						java.util.ArrayList<?> lista_parameter = (java.util.ArrayList<?>) fields[i].get(object);
						...
						for(int j=0; j<lista_this.size(); j++){
							if(lista_this.get(j)==null){
								...
								// SORT
								java.util.ArrayList<Object> lista_thisSORT = new java.util.ArrayList<Object>();
								java.util.ArrayList<Object> lista_parameterSORT = new java.util.ArrayList<Object>();
								try{
									java.util.ArrayList<String> lista_thisTMP = new java.util.ArrayList<String>();
									...
									for(int k=0; k<lista_thisTMP.size(); k++){
										String key = null;
										if(lista_thisTMP.get(k).getClass().getName().startsWith(this.getClass().getPackage().getName())){
											Class<?> c =  lista_thisTMP.get(k).getClass();
											java.lang.reflect.Method method =  c.getMethod("toString",boolean.class,boolean.class);
											key = (String) method.invoke(lista_thisTMP.get(k),false,true);
										}else{
											key = lista_thisTMP.get(k).toString();
										}
										Object thisObject = thisTmp.get(key);
										Object paramObject = parameterTmp.get(key);
										if(thisObject==null || paramObject==null){
											// significa che manca un elemento
											lista_thisSORT = (java.util.ArrayList<Object>) lista_this;
											lista_parameterSORT = (java.util.ArrayList<Object>) lista_parameter;
											break;
										}
		...
		try{
			...
			for(int i=0; i<fields.length;i++){
				...
				if(fields[i].get(this)==null){
					...
					if(fields[i].getType().getName().equals("java.util.ArrayList")){
						java.util.ArrayList<?> lista_this = (java.util.ArrayList<?>) fields[i].get(this);
						bf.append("ArrayList size("+lista_this.size()+")");
						...
						for(int j=0; j<lista_this.size(); j++){
							...
							if(lista_this.get(j).getClass().getName().startsWith(this.getClass().getPackage().getName())){
								Class<?> c =  lista_this.get(j).getClass();
								java.lang.reflect.Method method =  c.getMethod("toString",boolean.class,boolean.class);
								bf.append(method.invoke(lista_this.get(j),reportHTML,notIncludeID));
							}else{
								bf.append(lista_this.get(j).toString());
							}
							if(reportHTML) bf.append("<br>"); else bf.append("\n");
							bf.append("----end--- ");
							bf.append(this.getClass().getName());
