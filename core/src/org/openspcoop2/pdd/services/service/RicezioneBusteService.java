...
public class RicezioneBusteService implements IRicezioneService, IAsyncResponseCallback {
	...
	public void process(ConnectorInMessage reqParam, ConnectorOutMessage resParam, Date dataAccettazioneRichiesta, boolean async) throws ConnectorException {
		...
		try{
			...
			RicezioneBusteServiceUtils.emitTransaction(this.logCore, this.req, null, dataAccettazioneRichiesta, cInfo);
			return;
		}
		...
		if(cInfo!=null){
			this.res.close(false);
			RicezioneBusteServiceUtils.emitTransaction(this.context, this.logCore, this.req, pddContextFromServlet, dataAccettazioneRichiesta, cInfo);
		...
		if(ServiceBinding.SOAP.equals(this.requestInfo.getProtocolServiceBinding())){
			...
			if(method!=null && !HttpRequestMethod.POST.equals(method)){
				if(ServicesUtils.isRequestWsdl(this.req, this.logCore)) {
					...
					this.res.close(false);
					RicezioneBusteServiceUtils.emitTransaction(this.context, this.logCore, this.req, pddContextFromServlet, dataAccettazioneRichiesta, cInfoError);
					return;
				}
			}
		}		
