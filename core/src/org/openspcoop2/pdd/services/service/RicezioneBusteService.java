...
public class RicezioneBusteService implements IRicezioneService, IAsyncResponseCallback {
	...
	public void process(ConnectorInMessage req, ConnectorOutMessage res, Date dataAccettazioneRichiesta, boolean async) throws ConnectorException {
		...
		if (this.openSPCoopProperties != null && OpenSPCoop2Startup.initialize) {
			...
			try {
				...
				if(this.openSPCoopProperties.isConnettoriUseTimeoutInputStream()) {
					this.req.setRequestReadTimeout(this.openSPCoopProperties.getReadConnectionTimeout_ricezioneBuste());
				}
		...
		try{
			...
			RicezioneBusteServiceUtils.emitTransaction(this.logCore, req, null, dataAccettazioneRichiesta, cInfo);
			return;
		}
		...
		try{
			...
			ConnectorDispatcherErrorInfo cInfo = ConnectorDispatcherUtils.doError(this.requestInfo, this.generatoreErrore,
					ErroriIntegrazione.ERRORE_5XX_GENERICO_PROCESSAMENTO_MESSAGGIO.
						...
						IntegrationFunctionError.GOVWAY_NOT_INITIALIZED, e, null, res, this.logCore, ConnectorDispatcherUtils.GENERAL_ERROR);
			this.res.close(false);
			RicezioneBusteServiceUtils.emitTransaction(this.logCore, req, pddContextFromServlet, dataAccettazioneRichiesta, cInfo);
			return;
		}
		...
		ConnectorDispatcherInfo cInfo = RicezioneBusteServiceUtils.updatePortaApplicativaRequestInfo(this.requestInfo, this.logCore, req, res,
				this.generatoreErrore, serviceIdentificationReader,this.msgDiag, 
				this.context!=null ? this.context.getPddContext(): null);
		if(cInfo!=null){
			this.res.close(false);
			RicezioneBusteServiceUtils.emitTransaction(this.context, this.logCore, req, pddContextFromServlet, dataAccettazioneRichiesta, cInfo);
		...
		if(ServiceBinding.SOAP.equals(this.requestInfo.getProtocolServiceBinding())){
			...
			if(method!=null && !HttpRequestMethod.POST.equals(method)){
				if(ServicesUtils.isRequestWsdl(req, this.logCore)) {
					try {
						ServicesUtils.writeWsdl(res, this.requestInfo, RicezioneBusteConnector.ID_SERVICE, serviceIdentificationReader, this.logCore);
					}catch(Exception e) {
					...
					this.res.close(false);
					RicezioneBusteServiceUtils.emitTransaction(this.context, this.logCore, req, pddContextFromServlet, dataAccettazioneRichiesta, cInfoError);
					return;
				}
			}
		}		
		...
		try{
			...
			Map<String, String> configProperties = RicezioneBusteServiceUtils.readPropertiesConfig(this.req.getRequestInfo(), this.logCore,null);
			...
			if(ServiceBinding.SOAP.equals(protocolServiceBinding)){	
				if(this.openSPCoopProperties.isControlloContentTypeAbilitatoRicezioneBuste() == false){
			...
			if(!contentTypeSupportato){
				...
				/* ------------  SoapAction check 2 ------------- */
				if(soapAction!=null){
					if(this.openSPCoopProperties.checkSoapActionQuotedString_ricezioneBuste()){
						try{
			...
			if(this.context==null){
				...
				this.context = RicezioneBusteContext.newRicezioneBusteContext(this.idModuloAsService,dataAccettazioneRichiesta,this.requestInfo);
				this.context.setDataIngressoRichiesta(this.dataIngressoRichiesta);
				this.context.setTipoPorta(TipoPdD.APPLICATIVA);
				this.context.setIdModulo(this.idModulo);
				this.context.getPddContext().addObject(org.openspcoop2.core.constants.Costanti.PROTOCOL_NAME, this.protocolFactory.getProtocol());
			...
			try{
				if(sendInvoked==false) {
					...
					if( (this.responseMessage!=null && this.responseMessage.getParseException() != null) ||
						...
						responseMessageError = this.generatoreErrore.buildErroreProcessamento(this.pddContext, IntegrationFunctionError.INTERNAL_RESPONSE_ERROR,
