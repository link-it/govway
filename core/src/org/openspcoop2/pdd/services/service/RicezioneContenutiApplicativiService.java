...
public class RicezioneContenutiApplicativiService implements IRicezioneService, IAsyncResponseCallback {
	...
	public void process(ConnectorInMessage reqParam, ConnectorOutMessage resParam, Date dataAccettazioneRichiestaParam, boolean async) throws ConnectorException {
		...
		try{
			...
			this.logCore.error("req.getAttribute("+CostantiPdD.OPENSPCOOP2_PDD_CONTEXT_HEADER_HTTP+") read context from servlet failed: "+e.getMessage(),e);
		}
		...
		if(this.idTransazione!=null) {
			try {
				if(this.openSPCoopProperties.isTransazioniEnabled()) {
					TransactionContext.createTransaction(this.idTransazione, "RicezioneContenutiApplicativi.1");
				}
		...
		try{
			...
			if(ServiceBinding.SOAP.equals(integrationServiceBinding) &&
				!this.openSPCoopProperties.isControlloContentTypeAbilitatoRicezioneContenutiApplicativi() &&
			...
			if(!contentTypeSupportato){
				...
				/* ------------  SoapAction check 2 ------------- */
				if(soapAction!=null &&
					this.openSPCoopProperties.checkSoapActionQuotedString_ricezioneContenutiApplicativi()){
					try{
			...
			try{
				if(!sendInvoked) {
					...
					if( (this.responseMessage!=null && this.responseMessage.getParseException() != null) ||
						...
						responseMessageError = this.generatoreErrore.build(this.pddContext, ife,
								ErroriIntegrazione.ERRORE_426_SERVLET_ERROR.
					...
					esito = this.protocolFactory.createEsitoBuilder().getEsito(this.req.getURLProtocolContext(),
							statoServletResponse, this.requestInfo.getIntegrationServiceBinding(),
							responseMessageError, this.context.getProprietaErroreAppl(), informazioniErroriInfrastrutturaliError,
							this.pddContext);
				...
				try{
					...
					try{
						...
						this.logCore.error("Response.setStatus("+statoServletResponse+") set failed: "+eStatus.getMessage(),eStatus);
					}
