...
public class InoltroRisposte extends GenericLib{
		...
		try{
			...
			if(functionAsRouter==false){
				try{
					...
					pa = configurazionePdDManager.getPortaApplicativa_SafeMethod(idPA, null);
				}catch(Exception e){
			...
			if(inoltroSegnalazioneErrore==false){
				...
				if(responseMessage!=null){
					esitoHandler = protocolFactory.createEsitoBuilder().getEsito(responseMessage);			
	...
	private FlowProperties  getFlowProperties(Busta bustaRisposta,
			...
			org.openspcoop2.protocol.sdk.IProtocolFactory protocolFactory)throws DriverConfigurazioneException{
		...
		try{
			...
			if(bustaRisposta.getProfiloDiCollaborazione()==null&& bustaRisposta.sizeListaRiscontri()>0){
				if(bustaRisposta.getTipoServizioRichiedenteBustaDiServizio()!=null &&
					...
					PortaApplicativa pa = configurazionePdDManager.getPortaApplicativa_SafeMethod(idPA, null);
