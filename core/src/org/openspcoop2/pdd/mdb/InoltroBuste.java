...
public class InoltroBuste extends GenericLib implements IAsyncResponseCallback{
				...
				if(tipiIntegrazionePD!=null && tipiIntegrazionePD.length>0){
					...
					for (int i = 0; i < tipiIntegrazionePD.length; i++) {
						try {
							...
							String msg = "Inizializzazione gestore dati di integrazione (protocollo: "+protocol+") per le fruizioni di tipo "
	...
	private MessageSecurityConfig messageSecurityRequestConfig = null;
	...
	private EsitoLib processEngine(IOpenSPCoopState openspcoopstateParam,
		...
		this.requestMessagePrimaTrasformazione = null;
		...
		this.gestioneBusteNonRiscontrateAttive = false;
		if(!this.functionAsRouter){
			if(Costanti.SCENARIO_ONEWAY_INVOCAZIONE_SERVIZIO.equals(this.richiestaDelegata.getScenario()) && (!this.sendRispostaApplicativa) ){
				this.gestioneBusteNonRiscontrateAttive =consegnaAffidabile;
			}else if (
					(
							Costanti.SCENARIO_ASINCRONO_ASIMMETRICO_INVOCAZIONE_SERVIZIO.equals(this.richiestaDelegata.getScenario()) ||
							Costanti.SCENARIO_ASINCRONO_SIMMETRICO_INVOCAZIONE_SERVIZIO.equals(this.richiestaDelegata.getScenario()) ||
							Costanti.SCENARIO_ASINCRONO_SIMMETRICO_CONSEGNA_RISPOSTA.equals(this.richiestaDelegata.getScenario()) 
					)
					&&
					!this.richiestaDelegata.isRicevutaAsincrona()
				) {
		...
		if(this.functionAsRouter &&
			...
			&& (!this.portaDiTipoStateless) && (!routingStateless) ){
		...
		if( (!this.portaDiTipoStateless) && (!routingStateless)){
			// New connection puo' essere usata solo se non siamo in modalita stateless
				...
				if(org.openspcoop2.protocol.engine.constants.Costanti.MODIPA_PROTOCOL_NAME.equals(this.protocolFactory.getProtocol()) &&
					...
					if(modIValidazioneSemanticaProfiloSicurezza.isSicurezzaTokenOauth()) {
						...
						if(useJtiAuthorization) {
							...
							String id = this.bustaRichiesta.getProperty(CostantiDB.MODIPA_BUSTA_EXT_PROFILO_SICUREZZA_MESSAGGIO_ID);
		...
		try {
			...
			if(!this.invokerNonSupportato){
				...
				try {
					...
					if(this.transportResponseContext!=null) {
						this.addHeadersPeer(this.pddContext, this.transportResponseContext, props);
					}
			...
			if(this.propertiesReader.processHeaderIntegrazionePDResponse(this.functionAsRouter)){
				...
				if (tipiIntegrazionePDRisposta == null){
					if(InoltroBuste.defaultPerProtocolloGestoreIntegrazionePD.containsKey(this.protocolFactory.getProtocol())){
						tipiIntegrazionePDRisposta = InoltroBuste.defaultPerProtocolloGestoreIntegrazionePD.get(this.protocolFactory.getProtocol());
					}else{
						tipiIntegrazionePDRisposta = InoltroBuste.defaultGestoriIntegrazionePD;
					}
				}
					...
					try {
						...
						if(gestore!=null){
							...
							try {
								...
								throw new CoreException("Riscontrato errore durante l'inizializzazione della classe (IGestoreIntegrazionePD) ["+classType+
										"] da utilizzare per la gestione dell'integrazione (Risposta) delle fruizioni di tipo ["+tipiIntegrazionePDRisposta[i]+"]: "+e.getMessage());
							}
							if(this.responseMessage!=null ||
									( ! (gestore instanceof IGestoreIntegrazionePDSoap) )
									) {
							...
							this.msgDiag.logErroreGenerico("Lettura Gestore header di integrazione ["
											+ tipiIntegrazionePDRisposta[i]+ "]  non riuscita: non inizializzato", 
											"gestoriIntegrazionePD.get("+tipiIntegrazionePDRisposta[i]+")");
						}
					} catch (Exception e) {
						this.log.debug("Errore durante la lettura dell'header di integrazione ["+ tipiIntegrazionePDRisposta[i]
										+ "]: "+ e.getMessage(),e);
						this.msgDiag.addKeyword(CostantiPdD.KEY_TIPO_HEADER_INTEGRAZIONE,tipiIntegrazionePDRisposta[i]);
						this.msgDiag.addKeywordErroreProcessamento(e);
			...
			// Correlazione Applicativa
			if(!this.functionAsRouter && this.pd!=null && this.pd.getCorrelazioneApplicativaRisposta()!=null){
			...
			/* ------- Gestione punto 2 o 3 (MessaggioErroreProtocollo e MessaggioProtocollo) -------- */
				...
				if(this.functionAsRouter){ 
					...
					if(this.sendRispostaApplicativa && !isMessaggioErroreProtocollo && 
							erroriValidazione.isEmpty() &&
							erroriProcessamento.isEmpty()){
