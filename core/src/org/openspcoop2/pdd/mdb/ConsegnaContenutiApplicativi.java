...
public class ConsegnaContenutiApplicativi extends GenericLib implements IAsyncResponseCallback {
	...
	private EsitoLib engineOnMessage(OpenSPCoopStateDBManager dbManagerSource) throws OpenSPCoopStateException {
		...
		try{	    
			...
			/* ------------------- add start parameter -----------------------*/
			if(CostantiPdD.isEnabledAddSWAStartParameterIfNotPresent(this.consegnaMessageTrasformato)) {
				SoapUtils.addSWAStartParameterIfNotPresent(this.consegnaMessageTrasformato);
			}
		...
		try {
			...
			if(!this.invokerNonSupportato){
				...
				try {
					...
					if(this.transportResponseContext!=null) {
						this.addHeadersPeer(this.pddContext, this.transportResponseContext, this.proprietaPorta);
					}
