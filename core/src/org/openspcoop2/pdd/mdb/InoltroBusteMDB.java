	...
	public void onMessage(Message message) {
		if(message instanceof ObjectMessage) {
			...
			if( TimerMonitoraggioRisorse.risorseDisponibili == false){
				this.log.error("["+InoltroBuste.ID_MODULO+"] Risorse di sistema non disponibili: "+TimerMonitoraggioRisorse.risorsaNonDisponibile.getMessage(),TimerMonitoraggioRisorse.risorsaNonDisponibile);
				this.ctxMDB.setRollbackOnly();
				return;
			}
			...
			if( lib.getInizializzazioneUltimata() == false ){
				this.log = Logger.getLogger(InoltroBusteMDB.class);
