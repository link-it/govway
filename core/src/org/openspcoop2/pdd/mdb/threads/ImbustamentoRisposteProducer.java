 ...
 * @version $Rev$, $Date$
 */
public class ImbustamentoRisposteProducer extends ModuloAlternativoProducer implements	IProducer {
	
	private static String idModulo = "ImbustamentoRisposte";
	
	public ImbustamentoRisposteProducer(BlockingQueue<IWorker> coda) {
		super(ImbustamentoRisposteProducer.idModulo,coda);
	}
	...
	public void creaWorkers(Vector<MessageIde> messaggiTrovati) {
		for (int i=0;i<messaggiTrovati.size();i++){
			...
			ImbustamentoRisposteWorker task = new ImbustamentoRisposteWorker(ide);
			try {
				this.coda.put(task);
