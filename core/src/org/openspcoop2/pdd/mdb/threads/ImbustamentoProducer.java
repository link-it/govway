 ...
 * @version $Rev$, $Date$
 */
public class ImbustamentoProducer extends ModuloAlternativoProducer implements	IProducer {
	
	private static String idModulo = "Imbustamento";
	
	public ImbustamentoProducer(BlockingQueue<IWorker> coda) {
		super(ImbustamentoProducer.idModulo,coda);
	}
	...
	public void creaWorkers(Vector<MessageIde> messaggiTrovati) {
		for (int i=0;i<messaggiTrovati.size();i++){
			...
			ImbustamentoWorker task = new ImbustamentoWorker(ide);
			try {
				this.coda.put(task);
