...
/**
 * Implementazione JMX per la gestione dei dati della richiesta
 *   
...
public class GestoreRichieste extends NotificationBroadcasterSupport implements DynamicMBean {
	...
	public Object invoke(String actionName, Object[]params, String[]signature) throws MBeanException,ReflectionException{
		...
		if(actionName.equals(CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_GLOBALE)){
			return this.removeRateLimitingGlobalConfigCache();
		}
		
		if(actionName.equals(CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_API)){
			if(params.length != 2)
				throw new MBeanException(new Exception("["+CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_API+"] Lunghezza parametri non corretta: "+params.length));
			
			TipoPdD param1 = null;
			if(params[0]!=null && !"".equals(params[0])){
				String param1_tipo = (String)params[0];
				param1 = TipoPdD.toTipoPdD(param1_tipo);
				if(param1==null) {
					try {
						param1 = TipoPdD.valueOf(param1_tipo);
					}catch(Throwable t) {}
				}
				if(param1==null || (!TipoPdD.APPLICATIVA.equals(param1) && !TipoPdD.DELEGATA.equals(param1))) {
					throw new MBeanException(new Exception("["+CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_API+"] Primo parametro ("+param1_tipo+") non corretto; valori ammessi: "+
							TipoPdD.APPLICATIVA.getTipo()+","+TipoPdD.DELEGATA.getTipo()));
				}
			}
			...
			return this.removeRateLimitingAPIConfigCache(param1, param2);
		}
	...
	public MBeanInfo getMBeanInfo(){
		...
		// MetaData per l'operazione removeRateLimitingGlobalConfigCache
		MBeanOperationInfo removeRateLimitingGlobalConfigCache 
		= new MBeanOperationInfo(CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_GLOBALE,"Rimuove le informazioni riguardanti policy di controllo del traffico globali",
			null,
			String.class.getName(),
			MBeanOperationInfo.ACTION);
		
		// MetaData per l'operazione removeRateLimitingGlobalConfigCache
		MBeanOperationInfo removeRateLimitingAPIConfigCache 
		= new MBeanOperationInfo(CACHE_METHOD_NAME_REMOVE_DATI_CONTROLLO_TRAFFICO_API,"Rimuove le informazioni riguardanti policy di controllo del traffico dell'erogazione/fruizione indicata",
			new MBeanParameterInfo[]{
				new MBeanParameterInfo("tipoPdD",String.class.getName(),"Indicazione se il nome della porta indicato riguarda una porta delegata o applicativa"),
				new MBeanParameterInfo("nomePorta",String.class.getName(),"Nome porta"),
			},
			String.class.getName(),
			MBeanOperationInfo.ACTION);
		...
		MBeanOperationInfo[] operations = new MBeanOperationInfo[]{resetCacheOP,printStatCacheOP,disabilitaCacheOP,abilitaCacheParametriOP,listKeysCacheOP,getObjectCacheOP,removeObjectCacheOP,
				removeRateLimitingGlobalConfigCache, removeRateLimitingAPIConfigCache};
	...
	public GestoreRichieste(){
		...
		// Configurazione
		this.cacheAbilitata = org.openspcoop2.pdd.core.GestoreRichieste.isCacheAbilitata();
	...
	public String resetCache(){
		try{
			...
			org.openspcoop2.pdd.core.GestoreRichieste.resetCache();
	...
	public String printStatCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return org.openspcoop2.pdd.core.GestoreRichieste.printStatsCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String abilitaCache(){
		try{
			org.openspcoop2.pdd.core.GestoreRichieste.abilitaCache();
	...
	public String abilitaCache(Long dimensioneCache,Boolean algoritmoCacheLRU,Long itemIdleTime,Long itemLifeSecond){
		try{
			org.openspcoop2.pdd.core.GestoreRichieste.abilitaCache(dimensioneCache,algoritmoCacheLRU,itemIdleTime,itemLifeSecond);
			this.cacheAbilitata = true;
			return JMXUtils.MSG_ABILITAZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(e.getMessage(),e);
	...
	public void disabilitaCache() throws JMException{
		try{
			org.openspcoop2.pdd.core.GestoreRichieste.disabilitaCache();
	...
	public String listKeysCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return org.openspcoop2.pdd.core.GestoreRichieste.listKeysCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String getObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return org.openspcoop2.pdd.core.GestoreRichieste.getObjectCache(key);
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String removeObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			org.openspcoop2.pdd.core.GestoreRichieste.removeObjectCache(key);
	...
	public String removeRateLimitingGlobalConfigCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			org.openspcoop2.pdd.core.GestoreRichieste.removeRateLimitingConfigGlobale();
			return JMXUtils.MSG_RIMOZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
	...
	public String removeRateLimitingAPIConfigCache(TipoPdD tipoPdD, String nomePorta){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			org.openspcoop2.pdd.core.GestoreRichieste.removeRateLimitingConfig(tipoPdD, nomePorta);
			return JMXUtils.MSG_RIMOZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
