 ...
 * @version $Rev$, $Date$
 */
public class EngineKeystoreCaching extends NotificationBroadcasterSupport implements DynamicMBean {
	...
	/* Costruttore */
	public EngineKeystoreCaching(){
		this.log = OpenSPCoop2Logger.getLoggerOpenSPCoopCore();
				
		// Configurazione
		this.cacheAbilitata = GestoreKeystoreCaching.isCacheAbilitata();
	...
	public String resetCache(){
		try{
			...
			GestoreKeystoreCaching.resetCache();
	...
	public String printStatCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreKeystoreCaching.printStatsCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String abilitaCache(){
		try{
			GestoreKeystoreCaching.abilitaCache();
	...
	public String abilitaCache(Long dimensioneCache,Boolean algoritmoCacheLRU,Long itemIdleTime,Long itemLifeSecond){
		try{
			GestoreKeystoreCaching.abilitaCache(dimensioneCache,algoritmoCacheLRU,itemIdleTime,itemLifeSecond, this.log);
			this.cacheAbilitata = true;
			return JMXUtils.MSG_ABILITAZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(e.getMessage(),e);
	...
	public void disabilitaCache() throws JMException{
		try{
			GestoreKeystoreCaching.disabilitaCache();
	...
	public String listKeysCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreKeystoreCaching.listKeysCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String getObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreKeystoreCaching.getObjectCache(key);
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String removeObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			GestoreKeystoreCaching.removeObjectCache(key);
