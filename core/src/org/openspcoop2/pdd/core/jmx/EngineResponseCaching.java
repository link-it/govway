...
public class EngineResponseCaching extends NotificationBroadcasterSupport implements DynamicMBean {
	...
	public Object invoke(String actionName, Object[]params, String[]signature) throws MBeanException,ReflectionException{
		...
		if(actionName.equals(CACHE_METHOD_NAME_GET_OBJECT_BY_UUID)){
			
			if(params.length != 1)
				throw new MBeanException(new Exception("["+CACHE_METHOD_NAME_GET_OBJECT_BY_UUID+"] Lunghezza parametri non corretta: "+params.length));
			...
			return this.getObjectCacheByUUID(param1);
		}
		
		if(actionName.equals(CACHE_METHOD_NAME_REMOVE_OBJECT_BY_UUID)){
			
			if(params.length != 1)
				throw new MBeanException(new Exception("["+CACHE_METHOD_NAME_REMOVE_OBJECT_BY_UUID+"] Lunghezza parametri non corretta: "+params.length));
			...
			return this.removeObjectCacheByUUID(param1);
		}
	...
	public MBeanInfo getMBeanInfo(){
		...
		String description = "Risposte salvate che sono transitate sul Gateway ("+OpenSPCoop2Properties.getInstance().getVersione()+")";
		...
		// get by uuid
		MBeanOperationInfo getObjectByUUID 
			= new MBeanOperationInfo(CACHE_METHOD_NAME_GET_OBJECT_BY_UUID,"Recupera la risposta salvata in cache",
				new MBeanParameterInfo[]{
					new MBeanParameterInfo("key",String.class.getName(),"Identificativo della risposta in cache"),
				},
				String.class.getName(),
				MBeanOperationInfo.ACTION);
		
		// remove by uuid
		MBeanOperationInfo removeObjectByUUID 
			= new MBeanOperationInfo(CACHE_METHOD_NAME_REMOVE_OBJECT_BY_UUID,"Rimuove la risposta salvata in cache",
				new MBeanParameterInfo[]{
					new MBeanParameterInfo("key",String.class.getName(),"Identificativo della risposta in cache"),
				},
				String.class.getName(),
		...
		// Lista operazioni
		MBeanOperationInfo[] operations = new MBeanOperationInfo[]{getObjectByUUID,removeObjectByUUID, 
	...
	/* Costruttore */
	public EngineResponseCaching(){
		this.log = OpenSPCoop2Logger.getLoggerOpenSPCoopCore();
				
		// Configurazione
		this.cacheAbilitata = GestoreCacheResponseCaching.isCacheAbilitata();
	...
	public String resetCache(){
		try{
			...
			GestoreCacheResponseCaching.resetCache();
	...
	public String printStatCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreCacheResponseCaching.printStatsCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String abilitaCache(){
		try{
			GestoreCacheResponseCaching.abilitaCache();
	...
	public String abilitaCache(Long dimensioneCache,Boolean algoritmoCacheLRU,Long itemIdleTime,Long itemLifeSecond){
		try{
			GestoreCacheResponseCaching.abilitaCache(dimensioneCache,algoritmoCacheLRU,itemIdleTime,itemLifeSecond, this.log);
			this.cacheAbilitata = true;
			return JMXUtils.MSG_ABILITAZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(e.getMessage(),e);
	...
	public void disabilitaCache() throws JMException{
		try{
			GestoreCacheResponseCaching.disabilitaCache();
	...
	public String listKeysCache(){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreCacheResponseCaching.listKeysCache("\n");
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String getObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			return GestoreCacheResponseCaching.getObjectCache(key);
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
		}
	...
	public String removeObjectCache(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			GestoreCacheResponseCaching.removeObjectCache(key);
	...
	public String getObjectCacheByUUID(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			ResponseCached rc = GestoreCacheResponseCaching.getInstance().readByUUID(key);
			if(rc == null) {
	...
	public String removeObjectCacheByUUID(String key){
		try{
			if(this.cacheAbilitata==false)
				throw new Exception("Cache non abilitata");
			GestoreCacheResponseCaching.getInstance().removeByUUID(key);
			return JMXUtils.MSG_RIMOZIONE_CACHE_EFFETTUATA;
		}catch(Throwable e){
			this.log.error(JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage(),e);
			return JMXUtils.MSG_OPERAZIONE_NON_EFFETTUATA+e.getMessage();
