 ...
 * @version $Rev$, $Date$
 */
public class ConnettoreHTTPJavaFlow extends ConnettoreExtBaseHTTP {
	...
	/* Costruttori */
	public ConnettoreHTTPJavaFlow(){
		super();
	}
	public ConnettoreHTTPJavaFlow(boolean https){
		super(https);
	}
...
//						entityProducer = new ConnettoreHTTPCORE5_inputStreamEntityProducer(isRequest, ct, null, TransferLengthModes.TRANSFER_ENCODING_CHUNKED.equals(this.tlm));
			...
			// TBK
			ConnettoreHTTPJavaFlow_responseCallback responseCallback = new ConnettoreHTTPJavaFlow_responseCallback( this, request, httpBody );
			BodyHandler<Publisher<List<ByteBuffer>>> responseBodyHandler = BodyHandlers.ofPublisher();
			CompletableFuture< HttpResponse<Publisher<List<ByteBuffer>>> > sendAsync =
			...
			sendAsync.thenAccept( response -> {
									responseCallback.completed( new ConnettoreHTTPJavaFlow_responseConsumer( response, this.logger, dimensione_buffer, timeoutResponse ) );
								  })
					 .exceptionally( (t) -> {
						 			// TBK
						 			t.printStackTrace();
