...
public class ConnettoreStatus extends ConnettoreBaseWithResponse {
	...
	public static String getMessage(ConnettoreStatusResponseType statusResponseType, boolean isRest) {
		
		switch(statusResponseType) {
		case MODI:
			if (isRest) {
				return "{ \"type\": \"https://httpstatuses.com/200\", \"title\": \"Success\", \"status\": 200, \"detail\": \"Il servizio funziona correttamente\" }";
			} else {
				return "<?xml version='1.0' encoding='UTF-8'?>"
						+ "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\">"
						+ "<soapenv:Body>"
						+ "<problem xmlns=\"urn:ietf:rfc:7807\"> <type>https://httpstatuses.com/200</type> <title>Success</title> <status>200</status> <detail>Il servizio funziona correttamente</detail> </problem>"
						+ "</soapenv:Body>"
						+ "</soapenv:Envelope>";
			}
		case VUOTO: return "";
		case XML: return "Il servizio funziona correttamente";
		case JSON: return "{\"status\": 200, \"detail\": \"Il servizio funziona correttamente\"}";
		case TEXT: return "Il servizio funziona correttamente";
		default: return null;
		}
	}
	
	public static String getContentType(ConnettoreStatusResponseType statusResponseType, boolean isRest) {
		switch(statusResponseType) {
		case MODI:
			if (isRest) {
	...
	private void testConnectivityErogazione(Map<Long, String> connettoriServizio) throws DriverRegistroServiziException, DriverConfigurazioneException, DriverConfigurazioneNotFound {
		...
		for (MappingErogazionePortaApplicativa mapping : mappings) {
			...
			for (PortaApplicativaServizioApplicativo sa : serviziApplicativi) {						
				...
				if (servizioApplicativo.getInvocazioneServizio() != null && 
					...
					connettoriServizio.put(servizioApplicativo.getInvocazioneServizio().getConnettore().getId(), mapping.getDescrizione());
			}
		}
	}
	...
	private void testConnectivityFruizione(Map<Long, String> connettoriServizio) throws DriverRegistroServiziException, DriverConfigurazioneException, DriverConfigurazioneNotFound {
		...
		for (MappingFruizionePortaDelegata mapping : mappings) {
			...
			for (String azione : azioni.getAzioneDelegataList()) {
				...
				if (connettore != null && connettore.getId() != null)
					connettoriServizio.put(connettore.getId(), mapping.getDescrizione());
			}
		}
	}
	...
	private void testConnectivity() throws DriverRegistroServiziException, DriverConfigurazioneException, DriverConfigurazioneNotFound, ConnettoreException {
		...
		if (this.pa != null)
			this.testConnectivityErogazione(connettoriServizio);
		...
		for (Map.Entry<Long, String> entry : connettoriServizio.entrySet()) {
			try {
				...
				throw new ConnettoreException("il connettore [" + entry.getKey() + "] del gruppo: " + entry.getValue() + " check fallito: " + e.getMessage());
			}
		}
	}
	...
	protected boolean send(ConnettoreMsg request) {
		...
		try{
			...
			this.isResponse = new ByteArrayInputStream(ConnettoreStatus.getMessage(statusResponseType, this.isRest).getBytes());
			this.tipoRisposta = ConnettoreStatus.getContentType(statusResponseType, this.isRest);
