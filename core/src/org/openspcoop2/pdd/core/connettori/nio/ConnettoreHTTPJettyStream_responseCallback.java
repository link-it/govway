...
/**
 * ConnettoreHTTPJettyStream_responseCallback
 *
 *
 * @author $Author$
 * @version $Rev$, $Date$
 */
public class ConnettoreHTTPJettyStream_responseCallback {
	private ConnettoreHTTPJettyStream connettore;
	...
	public ConnettoreHTTPJettyStream_responseCallback( ConnettoreHTTPJettyStream connettore, ConnettoreMsg request, HttpBodyParameters httpBody ) {
	...
	public void completed( final ConnettoreHTTPJettyStream_responseConsumer responseParam ) {
		...
		try {
			...
			HttpFields hdrRisposta = responseParam.getHeaders();
			Map<String, List<String>> mapHeaderHttpResponse = new HashMap<String, List<String>>();
			if ( hdrRisposta != null ) {
				for ( HttpField curHeader : hdrRisposta ) {
					String key;

					if ( curHeader.getName() == null ) {
						// Check per evitare la coppia che ha come chiave null e come valore HTTP OK 200
						if ( this.connettore_debug )
							this.connettore_logger.debug("HTTP risposta ["+HttpConstants.RETURN_CODE+"] ["+ curHeader.getValue() +"]...");
						key = HttpConstants.RETURN_CODE;
					} else {
						if ( this.connettore_debug )
							this.connettore_logger.debug("HTTP risposta [" + curHeader.getName() +"] ["+ curHeader.getValue() +"]...");
						key = curHeader.getName();
					}
					if ( curHeader.getValues() != null ) {
						String[] values = curHeader.getValues();
						for ( int ix= 0; ix < values.length; ix++ )
							TransportUtils.addHeader( connettore_propertiesTrasportoRisposta, key, values[ix] );
						mapHeaderHttpResponse.put( key, List.of( values ) );
					}
				}
			}
			...
			if ( this.connettore.getCodiceTrasporto() >= 400 ) {
				...
				if ( this.connettore.getCodiceTrasporto() > 299 ) {
					...
					if(this.connettore.isFollowRedirects()){
						...
						org.openspcoop2.pdd.core.connettori.nio.ConnettoreHTTPJettyStream connettoreySyncRedirect = new org.openspcoop2.pdd.core.connettori.nio.ConnettoreHTTPJettyStream();
