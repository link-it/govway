...
/**
 * ConnettoreHTTPJettyStream
 *
 *
 * @author $Author$
 * @version $Rev$, $Date$
 */
public class ConnettoreHTTPJettyStream extends ConnettoreExtBaseHTTP {
	...
	/** Connessione */
	//protected ConnettoreHTTPJetty_connection httpClient = null;
	...
	public ConnettoreHTTPJettyStream(boolean https){
		super(https);
	}
...
//			}
			...
			ConnettoreHTTPJetty_connection httpClient = ConnettoreHTTPJetty_connectionManager.getConnettoreNIO(this.httpConnectionConfig, this.loader, this.logger, bfDebug);
			if(this.debug) {
				this.logger.debug("Creazione Connessione ...");
				this.logger.debug(bfDebug.toString());
			}

			this.httpRequest = httpClient.getHttpclient().newRequest( url.toString() );
...
//							entityProducer = new FileEntityProducer(bout.getSerializedFile(), ct, TransferLengthModes.TRANSFER_ENCODING_CHUNKED.equals(this.tlm));
							...
							this.httpRequest = this.httpRequest.body( new BytesRequestContent( bout.toByteArray() ) );
//							entityProducer = new BasicAsyncEntityProducer(bout.toByteArray(), ct, TransferLengthModes.TRANSFER_ENCODING_CHUNKED.equals(this.tlm));
					...
					if(hasContentRest) {
						...
						this.httpRequest = this.httpRequest.body( new InputStreamRequestContent( isRequest ) );
...
//						entityProducer = new ConnettoreHTTPCORE5_inputStreamEntityProducer(isRequest, ct, null, TransferLengthModes.TRANSFER_ENCODING_CHUNKED.equals(this.tlm));
			...
			switch (this.httpMethod) {
				case GET:
					this.httpRequest = this.httpRequest.method( HttpMethod.GET );
					break;
				case DELETE:
					this.httpRequest = this.httpRequest.method( HttpMethod.DELETE );
					break;
				case POST:
					this.httpRequest = this.httpRequest.method( HttpMethod.POST );
					break;
				case PUT:
					this.httpRequest = this.httpRequest.method( HttpMethod.PUT );
					break;
				case HEAD:
					this.httpRequest = this.httpRequest.method( HttpMethod.HEAD );
					break;
				case OPTIONS:
					this.httpRequest = this.httpRequest.method( HttpMethod.OPTIONS );
					break;
				case TRACE:
					this.httpRequest = this.httpRequest.method( HttpMethod.TRACE );
					break;
				case PATCH:
					this.httpRequest = this.httpRequest.method( HttpMethod.PATCH );
					break;
				case LINK:
					this.httpRequest = this.httpRequest.method( HttpMethod.LINK );
					break;
				case UNLINK:
					this.httpRequest = this.httpRequest.method( HttpMethod.UNLINK );
					break;
				default:
					this.httpRequest = this.httpRequest.method( this.httpMethod.toString() );
					break;
			}
...
//			this.httpRequest.timeout( totalReqTimeout, TimeUnit.SECONDS );
			...
			var listener = ( new Response.Listener() {
				...
				public void onFailure( Response response, Throwable failure ) {
					responseCallback.failed( failure );
				}
			} );
	...
	protected void setRequestHeader(String key, List<String> values) throws Exception {
    	if(values!=null && !values.isEmpty()) {
    		for (String value : values) {
    			this.httpRequest = this.httpRequest.headers( (hdrs) -> hdrs.add( key, value ) );		
			}
    	}
    }
}