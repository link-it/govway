...
public class GestoreTokenValidazioneUtilities {
	...
	private static boolean matchUrlInvocata(Logger log, AbstractDatiInvocazione datiInvocazione, UrlInvocazioneAPI urlInvocazione, List<String> prefixAggiuntivi,  List<String> baseUrlAggiuntive, 
		...
		try {
			...
			log.error("normalizeHtu ["+urlDefault+"] failed: "+e.getMessage(),e);
		}
	...
	private static void validazioneDPoPJti(DPoP dpop, IDPoPParser dpopParser, PolicyGestioneToken policyGestioneToken, Logger log) throws TokenException {
		...
		// DEBUG: Stampa statistiche cache JTI correnti
		System.out.println("ELIMINAMI");
		java.util.Map<String, com.github.benmanes.caffeine.cache.stats.CacheStats> allStats = LocalJtiCacheManager.getAllStats();
		if (!allStats.isEmpty()) {
			System.out.println("=== JTI Cache Stats ===");
			allStats.forEach((policy, stats) -> {
				// Ottiene cache per leggere dimensione reale (estimatedSize)
				com.github.benmanes.caffeine.cache.Cache<String, org.openspcoop2.pdd.core.token.dpop.jti.JtiEntry> cache =
					LocalJtiCacheManager.getCache(policy);
				long size = cache != null ? cache.estimatedSize() : 0;
