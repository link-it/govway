...
public class DynamicHelperCostanti {
	...
	public static final String LABEL_CONFIGURAZIONE_CLAIMS = LABEL_CONFIGURAZIONE_INFO_TRASPORTO_PREFIX+
			DOPPIO_A_CAPO_HTML+StringEscapeUtils.escapeHtml4("Un valore definito come parte dinamica tramite la sintassi ${..}, la cui risoluzione non è possibile a runtime, comporta un fallimento della richiesta in corso. Per una aggiunta del claim condizionata al riuscimento della risoluzione dinamica, senza far terminare la richiesta in errore in caso di mancata risoluzione, è possibile usare la forma '?{..}'.")+
			DOPPIO_A_CAPO_HTML+StringEscapeUtils.escapeHtml4("Fornendo un valore che inizia e termina con le parentesi graffe si definisce un oggetto json, come ad esempio:<BR/> claimTest={\"prova\":\"valoreProva\", \"prova2\":\"${header:X-Example}\"}")+
			DOPPIO_A_CAPO_HTML+StringEscapeUtils.escapeHtml4("Se il valore inizia e termina con le parentesi quadre si definisce invece un array json, come ad esempio:<BR/> claimTest=[\"valoreProva\", \"valoreProva2\", \"${header:X-Example}\"]")+
			DOPPIO_A_CAPO_HTML+StringEscapeUtils.escapeHtml4("Per definire tipi primitivi json (boolean,int,long,float,double) è necessario attuare un cast come ad esempio:<BR/> claimTest=cast(true as boolean) oppure claimTest=cast(${header:X-Example} as long)")+
	        DOPPIO_A_CAPO_HTML+StringEscapeUtils.escapeHtml4("Per convertire una lista json di tipi primitivi in lista di stringhe è possibile attuare un cast come ad esempio:<BR/> claimTest=cast([1,2,3] as string array)")+
	        DOPPIO_A_CAPO_HTML+LABEL_CONFIGURAZIONE_INFO_TRASPORTO_SUFFIX;
	
	public static final String LABEL_CONFIGURAZIONE_PURPOSE_ID = StringEscapeUtils.escapeHtml4("Il purposeId può essere configurato attraverso differenti modalità.")+
			DOPPIO_A_CAPO_HTML+
			StringEscapeUtils.escapeHtml4("Staticamente nella token policy dove viene indicato direttamente il valore; questa modalità richiede la registrazione di una token policy per ogni finalità.")+
			DOPPIO_A_CAPO_HTML+
			StringEscapeUtils.escapeHtml4("Dinamicamente indicando nella token policy un valore che indirizza proprietà degli oggetti del registro consentento di supportare differenti scenari:")+org.openspcoop2.core.constants.Costanti.WEB_NEW_LINE+
			StringEscapeUtils.escapeHtml4("- 1-1 con la fruizione: utilizzando il valore '${config:purposeId}' viene richiesta la registrazione della proprietà 'purposeId' nella fruizione dove viene utilizzata la policy.")+org.openspcoop2.core.constants.Costanti.WEB_NEW_LINE+
			StringEscapeUtils.escapeHtml4("- 1-1 con l'applicativo fruitore: viene richiesta la registrazione della proprietà 'purposeId' nell'applicativo fruitore, utilizzando il valore '${clientApplicationConfig:purposeId}' se l'applicativo viene identificato tramite l'autenticazione trasporto, o il valore '${tokenClientApplicationConfig:purposeId}' se viene identificato tramite il clientId presente nel token ottenuto dall'autenticazione token.")+org.openspcoop2.core.constants.Costanti.WEB_NEW_LINE+
			StringEscapeUtils.escapeHtml4("- N applicativi fruitore censiti sulla fruzione: è possibile registrare molteplici proprietà '&lt;clientApplicationName&gt;.purposeId' sulla fruizione, una per ogni applicativo fruitore il cui nome va indicato come prefisso della proprietà stessa, utilizzando il valore '${dynamicConfig:apiSearchByClientApplication(purposeId)}' se gli applicativi vengono identificati tramite l'autenticazione trasporto, o il valore '${dynamicConfig:apiSearchByTokenClientApplication(purposeId)}' se vengono identificati tramite il clientId presente nel token ottenuto dall'autenticazione token. È possibile utilizzare la proprietà senza prefisso come finalità di default.")+org.openspcoop2.core.constants.Costanti.WEB_NEW_LINE+
			StringEscapeUtils.escapeHtml4("- N fruizioni censite sull’applicativo fruitore: è possibile registrare molteplici proprietà '&lt;nomeApiImpl&gt;.v&lt;versioneApiImpl&gt;.purposeId' sull’applicativo, una per ogni fruizione di API che l’applicativo fruisce indicando il nome come prefisso della proprietà, utilizzando il valore '${dynamicConfig:clientApplicationSearch(purposeId)}' se l’applicativo viene identificato tramite l'autenticazione trasporto, o il valore '${dynamicConfig:tokenClientApplicationSearch(purposeId)}' se viene identificato tramite il clientId presente nel token ottenuto dall'autenticazione token. È possibile utilizzare la proprietà senza prefisso come finalità di default.")+
			DOPPIO_A_CAPO_HTML+
			StringEscapeUtils.escapeHtml4("Ogni applicativo fruitore viene censito con le credenziali relative ad una specifica modalità di autenticazione, ed è tramite tali credenziali che il Gateway lo identifica sulla richiesta: verificando le credenziali del trasporto (https, basic, api-key, principal) tramite l'autenticazione trasporto, oppure il clientId presente nel token tramite l'autenticazione token. Le keyword 'clientApplicationConfig', 'apiSearchByClientApplication' e 'clientApplicationSearch' riferiscono l'applicativo identificato nella prima modalità, mentre le corrispondenti 'tokenClientApplicationConfig', 'apiSearchByTokenClientApplication' e 'tokenClientApplicationSearch' riferiscono l'applicativo identificato nella seconda; non sono quindi interscambiabili e vanno scelte coerentemente con la modalità di identificazione attiva sulla fruizione.")+
			DOPPIO_A_CAPO_HTML+
			StringEscapeUtils.escapeHtml4("Contenere parti dinamiche risolte a runtime dal Gateway.")+LABEL_CONFIGURAZIONE_INFO_TRASPORTO_SUFFIX;
	...
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_ID_TRANSAZIONE = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_TRANSACTION_ID_OBJECT+"</b>: identificativo UUID della transazione ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_TRANSACTION_ID+")");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_DATA = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_DATE_OBJECT+"</b>: data di elaborazione del messaggio ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_DATE_OBJECT+")");
	...
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_BUSTA = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_BUSTA_OBJECT+"</b>: informazioni generiche del profilo ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_BUSTA_OBJECT+")");
	...
	/** public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_CONTEXT = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_CTX_OBJECT+"</b>: permette di accedere al contesto della richiesta ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_CTX_OBJECT_HTML_ESCAPED+")"); */
	...
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_INTEGRATION = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_INTEGRATION+"</b>: permette di accedere ai claims di un token di integrazione ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_INTEGRATION+")"); 
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_TOKEN_INFO = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_TOKEN_INFO+"</b>: permette di accedere ai claims di un token ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_TOKEN_INFO+")"); 
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_TOKEN_CLIENT = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_APPLICATIVO_TOKEN+"</b>: identità dell'applicativo client identificato tramite il clientId presente nel token ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_APPLICATIVO_TOKEN+")");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_ATTRIBUTES = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_ATTRIBUTES+"</b>: permette di accedere agli attributi recuperati tramite Attribute Authority ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_ATTRIBUTES+")"); 
	...
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_ATTACHMENTS = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_ATTACHMENTS_OBJECT+"</b>: consente di ottenere gli allegati registrati sull'API ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_ATTACHMENTS_OBJECT+")");
	...
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_ERROR_HANDLER = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_ERROR_HANDLER_OBJECT+"</b>: permette di generare risposte personalizzate che segnalano l'impossibilità di proseguire la trasformazione ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_ERROR_HANDLER_OBJECT+")");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_CLASS_FREEMARKER = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_CLASS_LOAD_STATIC+"</b>: permette di definire classi (es. class[\"org.apache.commons.lang3.StringUtils\"] ).");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_NEW_INSTANCE_FREEMARKER = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_CLASS_NEW_INSTANCE+"</b>: permette di istanziare una classe (es. new(\"java.lang.StringBuilder\",\"Commento Iniziale\") ).");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_CLASS_VELOCITY = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_CLASS_LOAD_STATIC+"</b>: permette di definire classi (es. class.forName(\"org.apache.commons.lang3.StringUtils\") ).");
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_NEW_INSTANCE_VELOCITY = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_CLASS_NEW_INSTANCE+"</b>: permette di istanziare una classe (es. new.instance(\"java.lang.StringBuilder\",\"Commento Iniziale\") ).");
	
	public static final String LABEL_CONFIGURAZIONE_INFO_OBJECT_RESPONSE_DATA = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_DATE_OBJECT+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_SUFFIX_RESPONSE+"</b>: data di elaborazione del messaggio di risposta ("+org.openspcoop2.pdd.core.dynamic.Costanti.TYPE_MAP_DATE_OBJECT+")");
...
//		LABEL_CONTROLLO_ACCESSI_AUTORIZZAZIONE_CONTENUTI_SOAP_VALORI.add(LABEL_CONTROLLO_ACCESSI_AUTORIZZAZIONE_CONTENUTI_FORM_VALUES);
	...
	public static final String LABEL_CONFIGURAZIONE_ATTRIBUTE_AUTHORITY_INFO_OBJECT_VALORI_REQUIRED_ATTRIBUTE = StringEscapeUtils.escapeHtml4("<b>"+org.openspcoop2.pdd.core.dynamic.Costanti.MAP_REQUIRED_ATTRIBUTES+"</b>: "+
	...
	public static final String LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO_DEFAULT_CLAIMS_REQUEST_CON_CLIENT_ID =
			"Se vengono indicati i claims 'iss', 'sub' o 'client_id' i valori forniti sovrascriveranno le impostazioni di default del prodotto.<br/>"+
				LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO_DEFAULT_NOT_GENERATED;
	...
	public static final String getLABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO(boolean request, List<String> denyClaims) {
		return getLABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO(request, denyClaims, false);
	}
	public static final String getLABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO(boolean request, List<String> denyClaims, boolean clientIdCustomConsentito) {
		...
		if(request && clientIdCustomConsentito) {
			sb.append(LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_JWT_CLAIMS_INFO_DEFAULT_CLAIMS_REQUEST_CON_CLIENT_ID);
		}
		else {
	...
	public static final String LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_ANY_VALUE = "<b>${anyValue}</b>: la verifica ha successo se il claim è presente nel token con un qualsiasi valore";
	public static final String LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_UNDEFINED = "<b>${undefined}</b>: la verifica ha successo se il claim non è presente nel token o non è valorizzato";
	...
	static {
		LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_AUDIENCE_INFO_VALORI_RESPONSE.add(LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_ANY_VALUE);
		LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_REST_AUDIENCE_INFO_VALORI_RESPONSE.add(LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_UNDEFINED);
	...
	static {
		LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_SOAP_AUDIENCE_INFO_VALORI_RESPONSE.add(LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_ANY_VALUE);
		LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_SOAP_AUDIENCE_INFO_VALORI_RESPONSE.add(LABEL_PARAMETRO_MODIPA_API_IMPL_PROFILO_SICUREZZA_MESSAGGIO_AUDIENCE_INFO_UNDEFINED);
