...
public class ErroreApplicativoUtilities {
	...
	private static void verificaFaultErroreApplicativo(Node erroreApplicativoNode,IDSoggetto dominioAtteso,TipoPdD tipoPdDAtteso,String[] identificativoModuloAtteso,
		...
		try{
			...
			Assert.assertTrue("http://www.openspcoop2.org/core/eccezione/errore_applicativo".equals(erroreApplicativoNode.getNamespaceURI()));
			...
			for(int i=0; i<listDettaglioOpenSPCoop.getLength(); i++){
				...
				if("ora-registrazione".equals(n.getLocalName())){
					...
					Assert.assertTrue( "http://www.openspcoop2.org/core/eccezione/errore_applicativo".equals(n.getNamespaceURI()) );
					...
					for(int j=0; j<listDominio.getLength(); j++){
						...
						if("identificativo-porta".equals(nDominio.getLocalName())){
							...
							Assert.assertTrue( "http://www.openspcoop2.org/core/eccezione/errore_applicativo".equals(nDominio.getNamespaceURI()) );
					...
					for(int j=0; j<listTipoEccezione.getLength(); j++){
						...
						if("codice".equals(tmp.getLocalName())){
							...
							findCodice = true;
							Assert.assertTrue( "http://www.openspcoop2.org/core/eccezione/errore_applicativo".equals(tmp.getNamespaceURI()) );
