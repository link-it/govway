...
public class RestTestEngine extends ConfigLoader {
	...
	private HttpResponse _test(
		...
		if(msgErrore!=null) {
			if(msgErrore.contains("Received fatal alert: bad_certificate")) {
				org.openspcoop2.core.protocolli.trasparente.testsuite.connettori.errori.RestTestEngine.verifyKo(response, "APIUnavailable", 503, "The API Implementation is temporary unavailable");
			}
			else {
				...
				org.openspcoop2.core.protocolli.trasparente.testsuite.connettori.errori.RestTestEngine.verifyKo(response, "AuthorizationContentDeny", 403, "Unauthorized request content");
			}
		}
		else {
