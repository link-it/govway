...
* @version $Rev$, $Date$
*/
public class AutorizzazioneContenutiPACustom extends org.openspcoop2.pdd.core.autorizzazione.pa.AutorizzazioneContenutoBuiltIn {
	...
	public EsitoAutorizzazionePortaApplicativa process(DatiInvocazionePortaApplicativa datiInvocazione, OpenSPCoop2Message msg) throws AutorizzazioneException {
		...
		try {
			...
			String tipoTest = Utilities.readTipoTest(datiInvocazione.getPa().getProprietaAutorizzazioneContenutoList(), true);
			Utilities.writeIdentificativoTest(id, tipoTest);
			applicativoAtteso = Utilities.readNomeApplicativo(datiInvocazione.getPa().getProprietaAutorizzazioneContenutoList(), true);
		}catch(Exception e) {
		...
		Utilities.addRegolaAutorizzazioneContenutoBuiltIn(datiInvocazione.getPa().getProprietaAutorizzazioneContenutoList());
