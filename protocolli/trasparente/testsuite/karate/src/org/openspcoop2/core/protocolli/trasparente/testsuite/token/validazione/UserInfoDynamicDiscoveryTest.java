...
public class UserInfoDynamicDiscoveryTest extends ConfigLoader {
	...
	static void test(TipoServizio tipoServizio, String api, String operazione) throws Exception {
		...
		try {
			...
			else if(UserInfoKeystoreSenzaPasswordTest.validazione_dynamic_discovery.equals(api)) {
				tokenPolicy = tokenPolicy + "_truststorePkcs12SenzaPassword";
			}
	...
	private static String buildDD(File f, String prefix, List<String> mapExpectedTokenInfo) {
		...
		String userInfoUri = org.openspcoop2.utils.Costanti.PROTOCOL_HTTPS_PREFIX+hostname+":8445/TestService/echo?existsQueryParameters=test_token&destFile="+f.getAbsolutePath()+"&destFileContentType=application/json";
