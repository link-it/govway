...
public class RestTestEngine extends ConfigLoader {
	...
	public void erogazioneInvalidContentTypeCaso1() throws Exception {
		test(TipoServizio.EROGAZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SMALL_SIZE).getBytes(),
				"invalidContentType1", 
	...
	public void fruizioneInvalidContentTypeCaso1() throws Exception {
		test(TipoServizio.FRUIZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SMALL_SIZE).getBytes(),
				"invalidContentType1", 
				"Il contenuto applicativo della risposta ricevuta non è processabile: Content-Type '\"application/json\";charset=UTF-8' presente nella risposta non valido: In Content-Type string <\"application/json\";charset=UTF-8>, expected MIME type, got application/json");
	}
	...
	private HttpResponse test(
		...
		if(operazione.startsWith("invalidContentType")) {
			verifyKo(response, INVALID_RESPONSE, 502, INVALID_RESPONSE_MESSAGE);
		}
		else {
		...
		try {
			...
			if(msgErrore.contains(";charset")) {
				try {
					...
					String msgTomcat = msgErrore.replace(";charset", "; charset");
	...
	public static void verifyKo(HttpResponse response, String error, int code, String errorMsg) throws Exception {
		...
		if(error!=null) {
			...
			try {
				...
				JSONObject jsonResp = JsonPathExpressionEngine.getJSONObject(new ByteArrayInputStream(response.getContent()));
