...
public class Utilities extends ConfigLoader {
	...
	public static String descr_notifica_ko_json = "{\"listaAnomalie\":[{\"codiceAnomalia\":\"0083\",\"descrizioneAnomalia\":\"Codice Classificazione non presente in base dati per attributo 9.2.2.1.0.0\",\"campiInErrore\":\"InizioRapporto.QualificaProfessionale\"},{\"codiceAnomalia\":\"0001\",\"descrizioneAnomalia\":\"La Qualifica Professionale '9.2.2.1.0.0' non è riconosciuta dalla corrispondente tabella di classificazione.\",\"campiInErrore\":\"InizioRapporto.QualificaProfessionale\"}]}";
	...
	public static HttpResponse _test(
		...
		if(msgErrore!=null) {
			if(api.contains("Rest2Soap")) {
				esitoExpected = EsitiProperties.getInstance(logCore, org.openspcoop2.protocol.engine.constants.Costanti.TRASPARENTE_PROTOCOL_NAME).convertoToCode(EsitoTransazioneName.HTTP_5xx);
			}
			else {
				esitoExpected = EsitiProperties.getInstance(logCore, org.openspcoop2.protocol.engine.constants.Costanti.TRASPARENTE_PROTOCOL_NAME).convertoToCode(EsitoTransazioneName.ERRORE_APPLICATIVO);
			}
