...
/**
 * Test di validazione REST per l'API {@code TestValidazioneRestOpenApi31External}: spec OpenAPI 3.1
 * con uno schema referenziato via {@code $ref} cross-file (allegato).
 * <p>
 * La porta principale usa il default per OpenAPI 3.1 ({@code kappa}). Le due risorse:
 * <ul>
 ...
 *       avviene a runtime nel motore kappa (patch 016, bridge json-sKema).</li>
 * </ul>
...
public class OpenApi31ExternalTest extends ConfigLoader {
	...
	private static final String API = "TestValidazioneRestOpenApi31External";
	...
	private void koRequestCase(TipoServizio tipo, String resource) throws Exception {
		...
		ValidazioneRestUtils.verifyKoRequest(response, ValidazioneRestUtils.DETAIL_KAPPA,
	...
	private void koResponseCase(TipoServizio tipo, String resource) throws Exception {
		...
		HttpResponse response = ValidazioneRestUtils.invokePost(url, VALID_BODY);
		ValidazioneRestUtils.verifyKoResponse(response, ValidazioneRestUtils.DETAIL_KAPPA,
