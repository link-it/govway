...
public class RestTestEngine extends ConfigLoader {
	...
	public static final String DIAGNOSTICO_CONNECTION_TIMEOUT_BIO = "connect timed out";
	public static final String DIAGNOSTICO_CONNECTION_TIMEOUT_NIO = "Connect to http://127.0.2.2:7777 [/127.0.2.2] failed: connect timed out after SOGLIA MILLISECONDS";
	
	public static final String DIAGNOSTICO_READ_TIMEOUT_BIO = "Read timed out";
	public static final String DIAGNOSTICO_READ_TIMEOUT_NIO = "Errore avvenuto durante la consegna HTTP: read timed out after SOGLIA MILLISECONDS";
	...
	public void erogazione_connectTimeout_globale() throws Exception {
		_test(TipoServizio.EROGAZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SMALL_SIZE).getBytes(),
				...
				this.mode.isNIO() ? DIAGNOSTICO_CONNECTION_TIMEOUT_NIO.replace(SOGLIA, "9000") : DIAGNOSTICO_CONNECTION_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_CONNECTION_TIMEOUT,
				MESSAGGIO_CONNECTION_TIMEOUT.replace(SOGLIA, "9000"), this.mode);
	}
	@Test
	public void fruizione_connectTimeout_globale() throws Exception {
		_test(TipoServizio.FRUIZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SMALL_SIZE).getBytes(),
				...
				this.mode.isNIO() ? DIAGNOSTICO_CONNECTION_TIMEOUT_NIO.replace(SOGLIA, "8000") : DIAGNOSTICO_CONNECTION_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_CONNECTION_TIMEOUT,
				MESSAGGIO_CONNECTION_TIMEOUT.replace(SOGLIA, "8000"), this.mode);
	}
	...
	public void erogazione_readTimeout_globale() throws Exception {
		_test(TipoServizio.EROGAZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SIZE_500K).getBytes(),
				...
				this.mode.isNIO() ? DIAGNOSTICO_READ_TIMEOUT_NIO.replace(SOGLIA, "16000") : DIAGNOSTICO_READ_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_READ_TIMEOUT,
				MESSAGGIO_READ_TIMEOUT.replace(SOGLIA, "16000"), this.mode);
	}
	@Test
	public void fruizione_readTimeout_globale() throws Exception {
		_test(TipoServizio.FRUIZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SIZE_500K).getBytes(),
				...
				Optional.empty(), // connettore       
				this.mode.isNIO() ? DIAGNOSTICO_READ_TIMEOUT_NIO.replace(SOGLIA, "15000") : DIAGNOSTICO_READ_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_READ_TIMEOUT,
				MESSAGGIO_READ_TIMEOUT.replace(SOGLIA, "15000"), this.mode);
	}
	...
	public void erogazione_connectTimeout_registrazioneAbilitata() throws Exception {
		_test(TipoServizio.EROGAZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SMALL_SIZE).getBytes(),
				...
				this.mode.isNIO() ? DIAGNOSTICO_CONNECTION_TIMEOUT_NIO.replace(SOGLIA, "10") : DIAGNOSTICO_CONNECTION_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_CONNECTION_TIMEOUT,
				MESSAGGIO_CONNECTION_TIMEOUT.replace(SOGLIA, "10"), this.mode);
	}
	@Test
	...
	public void erogazione_echoReceiveRequestSlow_registrazioneAbilitata() throws Exception {
		_test(TipoServizio.EROGAZIONE, HttpConstants.CONTENT_TYPE_JSON, Bodies.getJson(Bodies.SIZE_500K).getBytes(),
				...
				this.mode.isNIO() ? DIAGNOSTICO_READ_TIMEOUT_NIO.replace(SOGLIA, "2000") : DIAGNOSTICO_READ_TIMEOUT_BIO,
				TipoEvento.CONTROLLO_TRAFFICO_READ_TIMEOUT,
				MESSAGGIO_READ_TIMEOUT.replace(SOGLIA, "2000"), this.mode);
	}
	@Test
	...
	public static HttpResponse _testRest(
			...
			String policyName,
			Logger logCore, HttpLibraryMode mode) throws Exception {
		return _testRest(tipoServizio, contentType, content, operazione, tipoTest, gruppo, connettore,
				msgErrore != null ? Arrays.asList(msgErrore) : null,
				tipoEvento, descrizioneEvento, throttlingByte, throttlingMs, throttlingSend, applicativeId, policyName, logCore, mode);
	}
	...
	public static HttpResponse _testRest(
		...
		if (msgErrore != null && !msgErrore.isEmpty()) {
			if (msgErrore.size() == 1) {
				...
				msgPattern = Pattern.compile(".*" + Pattern.quote(msgErrore.get(0)) + ".*", Pattern.CASE_INSENSITIVE);
			} else {
				...
				patternBuilder.append(").*");
				msgPattern = Pattern.compile(patternBuilder.toString(), Pattern.CASE_INSENSITIVE);
			}
		}

		if (esitoTransazioneName.equals(EsitoTransazioneName.ERRORE_REQUEST_TIMEOUT)
				|| esitoTransazioneName.equals(EsitoTransazioneName.ERRORE_RESPONSE_TIMEOUT)
				|| esitoTransazioneName.equals(EsitoTransazioneName.ERRORE_CONNECTION_TIMEOUT))
