...
Examples:
| tipo-test | fruitore | username | password | response |
| esterno | DemoSoggettoFruitoreEsternoTestInterno | ApplicativoBlockingIDA01ExampleExternalClient1 | ApplicativoBlockingIDA01ExampleExternalClient1 | authorization-deny-canale-differente-applicativo-solo-certificato-intermediario-non-autorizzato.json |
...
@autorizzazioneSicurezzaMessaggioPuntualeVuotoKo
Scenario Outline: Test autorizzazione sicurezza messaggio puntuale caso ko (applicativo dominio <tipo-test>); nell'erogazione non è censito alcun applicativo
