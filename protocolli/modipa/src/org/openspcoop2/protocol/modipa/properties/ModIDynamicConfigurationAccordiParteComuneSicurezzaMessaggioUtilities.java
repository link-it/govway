...
public class ModIDynamicConfigurationAccordiParteComuneSicurezzaMessaggioUtilities {
	...
	static void addProfiloSicurezzaMessaggio(ModIProperties modiProperties,
		...
		if(rest) {
			...
			StringConsoleItem profiloSicurezzaMessaggioHeaderCustomModeItem = (StringConsoleItem) 
					ProtocolPropertiesFactory.newConsoleItem(ConsoleItemValueType.STRING,
					ConsoleItemType.SELECT,
			...
			profiloSicurezzaMessaggioHeaderCustomModeItem.addLabelValue(ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_LABEL_ALWAYS,
					ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_VALUE_ALWAYS);
			profiloSicurezzaMessaggioHeaderCustomModeItem.addLabelValue(ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_LABEL_RICHIESTE_CON_PAYLOAD_HTTP_QUALSIASI_RISPOSTA,
					ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_VALUE_RICHIESTE_CON_PAYLOAD_HTTP_QUALSIASI_RISPOSTA);
			profiloSicurezzaMessaggioHeaderCustomModeItem.addLabelValue(ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_LABEL_RISPOSTE_CON_PAYLOAD_HTTP_QUALSIASI_RICHIESTA,
					ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_VALUE_RISPOSTE_CON_PAYLOAD_HTTP_QUALSIASI_RICHIESTA);
			profiloSicurezzaMessaggioHeaderCustomModeItem.setDefaultValue(ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_DEFAULT_VALUE);
			profiloSicurezzaMessaggioHeaderCustomModeItem.setType(ConsoleItemType.HIDDEN);
			configuration.addConsoleItem(profiloSicurezzaMessaggioHeaderCustomModeItem);
		...
		BooleanConsoleItem profiloSicurezzaMessaggioCorniceSicurezzaOpzionale = (BooleanConsoleItem) 
				ProtocolPropertiesFactory.newConsoleItem(ConsoleItemValueType.BOOLEAN,
				ConsoleItemType.CHECKBOX,
				ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_CORNICE_SICUREZZA_OPZIONALE_ID,
				ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_CORNICE_SICUREZZA_OPZIONALE_LABEL);
	...
	static void updateProfiloSicurezzaMessaggio(ModIProperties modiProperties,
		...
		if(rest) {
			...
			if(isSicurezza01 || isSicurezza02 || isSicurezza03 || isSicurezza04) {
				...
				if(custom) {
					...
					if(profiloSicurezzaMessaggioHeaderCustomModeItem!=null) {
						if(isSicurezza03or04) {
							/***Rilascio con issue 1625 */ 
							profiloSicurezzaMessaggioHeaderCustomModeItem.setType(ConsoleItemType.SELECT);
						}
						else {
							profiloSicurezzaMessaggioHeaderCustomModeItem.setType(ConsoleItemType.HIDDEN);
							StringProperty profiloSicurezzaMessaggioHeaderCustomModeItemValue = (StringProperty) ProtocolPropertiesUtils.getAbstractPropertyById(properties, ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_HEADER_CUSTOM_MODE_ID);
		...
		if(sicurezzaSullaRichiesta && infoUtenteAbilitata) {
			...
			if(profiloSicurezzaMessaggioCorniceSicurezzaSchemaItem!=null) {
				...
				if(listConfig!=null && !listConfig.isEmpty()) {
					...
					if(profiloSicurezzaMessaggioCorniceSicurezzaPatternItemValue!=null && profiloSicurezzaMessaggioCorniceSicurezzaPatternItemValue.getValue()!=null &&
						...
						String schemaSelected = profiloSicurezzaMessaggioCorniceSicurezzaSchemaItemValue!=null ? profiloSicurezzaMessaggioCorniceSicurezzaSchemaItemValue.getValue() : null;
						if( (schemaSelected==null || StringUtils.isEmpty(schemaSelected)) && profiloSicurezzaMessaggioCorniceSicurezzaSchemaItemValue!=null) {
							schemaSelected = listConfig.get(0).getNome();
							profiloSicurezzaMessaggioCorniceSicurezzaSchemaItemValue.setValue(schemaSelected);
						}
			...
			if(profiloSicurezzaMessaggioCorniceSicurezzaOpzionaleItem!=null) {
				...
				String pattern = profiloSicurezzaMessaggioCorniceSicurezzaPatternItemValue!=null ? profiloSicurezzaMessaggioCorniceSicurezzaPatternItemValue.getValue() : null;
				if(pattern==null || ModIConsoleCostanti.MODIPA_PROFILO_SICUREZZA_MESSAGGIO_CORNICE_SICUREZZA_PATTERN_VALUE_OLD.equals(profiloSicurezzaMessaggioCorniceSicurezzaPatternItemValue.getValue())) {
