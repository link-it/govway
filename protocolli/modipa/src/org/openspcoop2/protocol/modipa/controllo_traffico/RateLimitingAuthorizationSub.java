 ...
 * @version $Rev$, $Date$
 */
public class RateLimitingAuthorizationSub extends AbstractModIRateLimiting {
	...
	protected String estraiValore(Logger log, SecurityToken securityToken, Dati datiRichiesta) throws PluginsException {
		try {
			...
			String sub = authorization.getPayloadClaim(CLAIM_SUB);
			if(sub == null) {
				throw new PluginsException("Claim '" + CLAIM_SUB + "' non presente nel token authorization ModI");
			}
			return sub;
