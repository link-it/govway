 ...
 * @version $Rev$, $Date$
 */
public class RateLimitingIntegrityClientId extends AbstractModIRateLimiting {
	...
	protected String estraiValore(Logger log, SecurityToken securityToken, Dati datiRichiesta) throws PluginsException {
		try {
			RestMessageSecurityToken integrity = getIntegrityToken(securityToken);
			String clientId = integrity.getPayloadClaim(CLAIM_CLIENT_ID);
			if(clientId == null) {
				throw new PluginsException("Claim '" + CLAIM_CLIENT_ID + "' non presente nel token integrity ModI");
			}
			return clientId;
