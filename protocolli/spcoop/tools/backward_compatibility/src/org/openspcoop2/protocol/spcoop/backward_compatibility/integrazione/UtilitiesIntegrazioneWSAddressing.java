...
public class UtilitiesIntegrazioneWSAddressing {
	...
	/**
	 * // NOTA: Tutti i tipi sono assunti come SPC
	   SOAPHeaderElement wsaTO      ->     http://<nomeSoggettoErogatore>.spcoop.it/servizi/<nomeServizio>
	   SOAPHeaderElement wsaFROM    ->     http://[<nomeServizioApplicativoFruitore>.]<nomeSoggettoFruitore>.spcoop.it
	   SOAPHeaderElement wsaAction  ->     http://<nomeSoggettoErogatore>.spcoop.it/servizi/<nomeServizio>/<nomeAzione>
	   SOAPHeaderElement wsaID      ->     uuid:<IDEGOV> in caso di restituzione di una risposto lato PD o in caso di consegna tramite PA
										   uuid:<IDApplicativo> in caso di invocazione lato PD (Da utilizzare per correlazione applicativa)
	   SOAPHeaderElement wsaRelatesTo = null; // uuid:<IDEGOV> equivale al riferimento messaggio

	 */
	private final static String WSA_TO_FORMAT = "http://<nomeSoggettoErogatore>.spcoop.it/servizi/<nomeServizio>";
	private final static String WSA_FROM_FORMAT = "http://[<nomeServizioApplicativoFruitore>.]<nomeSoggettoFruitore>.spcoop.it";
	private final static String WSA_ACTION_FORMAT = "http://<nomeSoggettoErogatore>.spcoop.it/servizi/<nomeServizio>/<nomeAzione>";
	...
	public static void readDatiWSATo(String wsaTO,HeaderIntegrazione integrazione) throws HeaderIntegrazioneException{
		...
		if(wsaTO.contains(".spcoop.it/servizi/")==false)
			throw new HeaderIntegrazioneException("WSATO Value is not valid: "+WSA_TO_FORMAT);
	...
	public static String buildDatiWSATo(String tipoSoggettoErogatore,String nomeSoggettoErogatore,String tipoServizio,String nomeServizio){
		return "http://"+nomeSoggettoErogatore+".spcoop.it/servizi/"+nomeServizio;
	}
	...
	public static void readDatiWSAFrom(String wsaFrom,HeaderIntegrazione integrazione) throws HeaderIntegrazioneException{
		...
		if(wsaFrom.contains(".spcoop.it")==false)
			throw new HeaderIntegrazioneException("WSAFrom Value is not valid: "+WSA_FROM_FORMAT);
		
		wsaFrom = wsaFrom.substring(7, wsaFrom.length());
		
		int indexSoggetto = wsaFrom.indexOf(".spcoop.it");
		...
		if(nomeSoggetto.contains(".")){
			...
			if(nomeSoggetto==null){
				throw new HeaderIntegrazioneException("WSAFrom Value is not valid (Soggetto non identificabile dopo parsing ServizioApplicativo): "+WSA_FROM_FORMAT);
			}	
			if(sa==null){
	...
	public static String buildDatiWSAFrom(String servizioApplicativoFruitore,String tipoSoggetto,String nomeSoggetto){
		if(servizioApplicativoFruitore==null)
			return "http://"+nomeSoggetto+".spcoop.it";
		else
			return "http://"+servizioApplicativoFruitore+"."+nomeSoggetto+".spcoop.it";
	}
	...
	public static void readDatiWSAAction(String wsaAction,HeaderIntegrazione integrazione) throws HeaderIntegrazioneException{
		...
		if(wsaAction.contains(".spcoop.it/")==false)
			throw new HeaderIntegrazioneException("WSAAction Value is not valid: "+WSA_ACTION_FORMAT);
		
		// soggetto
		int indexSoggetto = wsaAction.indexOf(".spcoop.it/servizi/");
		...
		// servizio
		String servizio = wsaAction.substring((indexSoggetto+".spcoop.it/servizi/".length()), wsaAction.length());
		if(servizio==null){
	...
	public static String buildDatiWSAAction(String tipoSoggettoErogatore,String nomeSoggettoErogatore,String tipoServizio,String nomeServizio,String azione){
		return "http://"+nomeSoggettoErogatore+".spcoop.it/servizi/"+nomeServizio+"/"+azione;
	}
