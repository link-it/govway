...
public class RicezioneContenutiApplicativiDirect extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		try{
			...
			if(generateErrorMessage){
				ConnectorUtils.generateErrorMessage(IDService.PORTA_DELEGATA_SOAP, MethodType.POST, req, res, e.getMessage(), false, true);
			}
			else{
				res.sendError(404, ConnectorUtils.generateError404Message(ConnectorUtils.getFullCodeProtocolUnsupported(IDService.PORTA_DELEGATA_SOAP)));
			}
			return;
		}
		try{
	...
	throws ServletException, IOException {
		...
		try{
			...
			if(generateErrorMessage){
				ConnectorUtils.generateErrorMessage(IDService.PORTA_DELEGATA_SOAP, method, req, res, e.getMessage(), false, true);
			}
			else{
				res.sendError(404, ConnectorUtils.generateError404Message(ConnectorUtils.getFullCodeProtocolUnsupported(IDService.PORTA_DELEGATA_SOAP)));
			}
